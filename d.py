...
# Enhanced Configuration with Heart Attack Prediction
@dataclass
class Config:
    ...
    heart_attack_pkl_path: str = "heart_attack_model.pkl"
    heart_attack_scaler_path: str = "heart_attack_scaler.pkl"
    heart_attack_features: List[str] = None
    heart_attack_threshold: float = 0.5
    
    def __post_init__(self):
        if self.heart_attack_features is None:
            # Default feature set for heart attack prediction
            self.heart_attack_features = [
...
class HealthAnalysisAgent:
    def __init__(self, custom_config: Optional[Config] = None):
        ...
        logger.info("🔧 HealthAnalysisAgent initialized with Snowflake Cortex API + Interactive Chatbot + Heart Attack Prediction")
        logger.info(f"🌐 API URL: {self.config.api_url}")
        logger.info(f"🤖 Model: {self.config.model}")
        logger.info(f"🔑 App ID: {self.config.app_id}")
        logger.info(f"💬 Chatbot: Enhanced Interactive Mode")
        logger.info(f"❤️ Heart Attack Prediction: Enabled")
        logger.info(f"📁 PKL Model Path: {self.config.heart_attack_pkl_path}")
        
        # Load heart attack prediction model
        self.heart_attack_model = None
        self.heart_attack_scaler = None
        self._load_heart_attack_model()
        
        self.setup_langgraph()
    
    def _load_heart_attack_model(self):
        """Load the heart attack prediction model from PKL file"""
        try:
            logger.info(f"📁 Loading heart attack prediction model from: {self.config.heart_attack_pkl_path}")
            
            if not os.path.exists(self.config.heart_attack_pkl_path):
                logger.warning(f"⚠️ Heart attack model file not found: {self.config.heart_attack_pkl_path}")
                logger.warning("⚠️ Heart attack prediction will be disabled")
                return
            
            with open(self.config.heart_attack_pkl_path, 'rb') as f:
                self.heart_attack_model = pickle.load(f)
            
            logger.info("✅ Heart attack prediction model loaded successfully")
            logger.info(f"🤖 Model type: {type(self.heart_attack_model).__name__}")
            
            # Try to load scaler if available
            try:
                if os.path.exists(self.config.heart_attack_scaler_path):
                    with open(self.config.heart_attack_scaler_path, 'rb') as f:
                        self.heart_attack_scaler = pickle.load(f)
                    logger.info("✅ Heart attack scaler loaded successfully")
                else:
                    logger.info("ℹ️ No scaler file found, using model without scaling")
            except Exception as e:
                logger.warning(f"⚠️ Could not load scaler: {e}")
                
        except Exception as e:
            logger.error(f"❌ Error loading heart attack model: {e}")
    ...
    def setup_langgraph(self):
        ...
        logger.info("✅ Enhanced LangGraph workflow compiled successfully with 8 nodes including heart attack prediction!")
    ...
    def predict_heart_attack(self, state: HealthAnalysisState) -> HealthAnalysisState:
        """LangGraph Node 8: Predict heart attack risk using ML model and health data"""
        logger.info("❤️ LangGraph Node 8: Starting heart attack prediction...")
        state["current_step"] = "predict_heart_attack"
        state["step_status"]["predict_heart_attack"] = "running"
        
        try:
            if self.heart_attack_model is None:
                logger.warning("⚠️ Heart attack model not available, skipping prediction")
                state["heart_attack_prediction"] = {
                    "error": "Heart attack model not loaded",
                    "risk_score": 0.0,
            ...
            features = self._extract_heart_attack_features(state)
            state["heart_attack_features"] = features
            
            if not features or "error" in features:
                state["errors"].append("Failed to extract features for heart attack prediction")
            ...
            if feature_vector is None:
                state["errors"].append("Failed to prepare feature vector for prediction")
                state["step_status"]["predict_heart_attack"] = "error"
                return state
            
            # Make prediction
            prediction_result = self._make_heart_attack_prediction(feature_vector, state)
            ...
            logger.info("✅ Heart attack prediction completed successfully")
    ...
    def initialize_chatbot(self, state: HealthAnalysisState) -> HealthAnalysisState:
        ...
        logger.info("💬 LangGraph Node 9: Initializing interactive chatbot with heart attack prediction...")
        ...
        try:
            ...
            logger.info("✅ Successfully initialized interactive chatbot with heart attack prediction context")
            logger.info(f"💬 Chatbot ready with {len(chatbot_context)} data components including heart attack prediction")
    ...
    # ===== HEART ATTACK PREDICTION METHODS =====
    
    def _extract_heart_attack_features(self, state: HealthAnalysisState) -> Dict[str, Any]:
        """Extract relevant features for heart attack prediction from health data"""
        try:
            logger.info("🔍 Extracting features for heart attack prediction...")
            ...
            features["sex"] = 1 if gender == "M" else 0  # 1 for Male, 0 for Female
            ...
            features["fbs"] = 1 if diabetes == "yes" else 0  # Fasting blood sugar > 120 mg/dl
            
            # Blood pressure indicator
            blood_pressure = entity_extraction.get("blood_pressure", "unknown")
            if blood_pressure in ["managed", "diagnosed"]:
                features["trestbps"] = 140  # Elevated blood pressure
            else:
                features["trestbps"] = 120  # Normal blood pressure
            
            # Smoking indicator
            smoking = entity_extraction.get("smoking", "no")
            features["exang"] = 1 if smoking == "yes" else 0  # Exercise induced angina
            
            # Extract from medical extraction (ICD-10 codes)
            medical_extraction = state.get("medical_extraction", {})
            hlth_srvc_records = medical_extraction.get("hlth_srvc_records", [])
            
            # Analyze diagnosis codes for heart-related conditions
            heart_related_codes = []
            chest_pain_codes = []
            ...
            for record in hlth_srvc_records:
                ...
                for diag in diagnosis_codes:
                    code = diag.get("code", "")
                    if code:
                        # Heart disease codes (I20-I25)
                        if code.startswith(("I20", "I21", "I22", "I23", "I24", "I25")):
                            heart_related_codes.append(code)
                        # Chest pain codes (R06.02, R06.03)
                        if code.startswith(("R06", "R07")):
                            chest_pain_codes.append(code)
            
            # Chest pain type (cp)
            if heart_related_codes:
                features["cp"] = 1  # Atypical angina
            elif chest_pain_codes:
                features["cp"] = 2  # Non-anginal pain
            else:
                features["cp"] = 0  # Typical angina (default)
            
            # Extract from pharmacy data for heart medications
            pharmacy_extraction = state.get("pharmacy_extraction", {})
            ...
            for record in ndc_records:
                ...
                if lbl_nm:
                    # Heart medications
                    if any(med in lbl_nm for med in ["metoprolol", "lisinopril", "atorvastatin", "simvastatin"]):
                        heart_medications.append(lbl_nm)
                    # Cholesterol medications
                    if any(med in lbl_nm for med in ["atorvastatin", "simvastatin", "lovastatin", "pravastatin"]):
                        cholesterol_medications.append(lbl_nm)
            
            # Cholesterol estimate
            if cholesterol_medications:
                features["chol"] = 240  # High cholesterol (on medication)
            else:
                features["chol"] = 200  # Normal cholesterol
            ...
            feature_summary = {
                ...
                "feature_sources": {
                    ...
                    "sex": "patient_data",
                    "diabetes": "entity_extraction",
                    "blood_pressure": "entity_extraction",
                    "smoking": "entity_extraction",
                    "heart_conditions": f"{len(heart_related_codes)} ICD-10 codes found",
                    "chest_pain": f"{len(chest_pain_codes)} chest pain codes found",
                    "heart_medications": f"{len(heart_medications)} medications found",
                    "cholesterol_medications": f"{len(cholesterol_medications)} medications found"
                },
                "feature_interpretation": {
                    "age": f"{features['age']} years old",
                    "sex": "Male" if features["sex"] == 1 else "Female",
                    "diabetes": "Yes" if features["fbs"] == 1 else "No",
                    "high_bp": "Yes" if features["trestbps"] > 130 else "No",
                    "smoking_indicator": "Yes" if features["exang"] == 1 else "No",
                    "chest_pain_type": ["Typical angina", "Atypical angina", "Non-anginal pain", "Asymptomatic"][min(features["cp"], 3)],
                    "cholesterol_level": "High" if features["chol"] > 200 else "Normal"
                }
            }
            
            logger.info(f"✅ Extracted {len(features)} features for heart attack prediction")
            logger.info(f"📊 Key features: Age={features['age']}, Sex={'M' if features['sex']==1 else 'F'}, Diabetes={'Y' if features['fbs']==1 else 'N'}")
    ...
    def _prepare_feature_vector(self, features: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for feature_name in self.config.heart_attack_features:
            ...
            logger.info(f"✅ Feature vector prepared: shape {feature_array.shape}")
            logger.info(f"📊 Feature values: {feature_array[0][:5]}...")
    ...
    def _make_heart_attack_prediction(self, feature_vector: np.ndarray, state: HealthAnalysisState) -> Dict[str, Any]:
        """Make heart attack prediction using the loaded ML model"""
        try:
            logger.info("🤖 Making heart attack prediction...")
            ...
            prediction_result = {
                ...
                "prediction_interpretation": {
                    "risk_assessment": f"The ML model predicts a {risk_level} risk of heart attack",
                ...
                "model_info": {
                    ...
                    "features_used": len(self.config.heart_attack_features),
                    "threshold": self.config.heart_attack_threshold,
                    "prediction_timestamp": datetime.now().isoformat()
                }
            }
            
            logger.info(f"✅ Heart attack prediction completed")
            ...
            logger.error(f"Error making heart attack prediction: {e}")
            return {
    ...
    def _identify_risk_factors(self, state: HealthAnalysisState) -> List[str]:
        ...
        # Gender risk
        if extracted_features.get("sex", 0) == 1:
            risk_factors.append("Male gender")
        
        # Diabetes
        if extracted_features.get("fbs", 0) == 1:
            risk_factors.append("Diabetes mellitus")
        
        # Hypertension
        if extracted_features.get("trestbps", 0) > 140:
            risk_factors.append("Hypertension")
        
        # High cholesterol
        if extracted_features.get("chol", 0) > 240:
            risk_factors.append("High cholesterol")
        
        # Smoking
        if extracted_features.get("exang", 0) == 1:
            risk_factors.append("Smoking/Exercise intolerance")
        
        # Chest pain
        cp_type = extracted_features.get("cp", 0)
        if cp_type > 0:
            risk_factors.append("Chest pain history")
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Handle chatbot conversation with deidentified medical data context including heart attack prediction"""
        try:
            ...
            # Create comprehensive prompt with medical and heart attack prediction context
            full_prompt = f"""{self.config.chatbot_sys_msg}

Here are the complete deidentified medical records, analysis, and heart attack risk prediction for this patient:
...
3. Heart attack risk assessment and contributing factors
4. Potential health implications
5. Professional medical insights based on the available data

Always maintain patient privacy and provide evidence-based responses using the medical data and ML predictions provided."""

            logger.info(f"💬 Processing chatbot query with heart attack prediction context: {user_query[:100]}...")
...
"""
    ...
    def run_analysis(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """Run the enhanced health analysis workflow using LangGraph with heart attack prediction"""
        ...
        try:
            ...
            logger.info("🚀 Starting Enhanced LangGraph health analysis workflow with heart attack prediction...")
            ...
            logger.info(f"❤️ Heart Attack Prediction: ML model enabled")
            ...
            results = {
                ...
                # NEW: Heart attack prediction results
            ...
            if results["success"]:
                logger.info("✅ Enhanced LangGraph health analysis with heart attack prediction completed successfully!")
...
def main():
    """Example usage of the Enhanced LangGraph Health Analysis Agent with Heart Attack Prediction"""
    
    print("🏥 Enhanced LangGraph Health Analysis Agent v4.0")
    print("✅ Agent is ready with Snowflake Cortex API + Interactive Chatbot + Heart Attack Prediction")
    print("❤️ Heart Attack Prediction using ML model from PKL file")
    print("🔧 To use this agent, run: streamlit run streamlit_langgraph_ui.py")
    print()
    
    # Show configuration including heart attack prediction
    config = Config(
        heart_attack_pkl_path="heart_attack_model.pkl",  # User will specify this
        heart_attack_scaler_path="heart_attack_scaler.pkl",  # Optional
    ...
    print(f"   ❤️ Heart Attack Model: {config.heart_attack_pkl_path}")
    print(f"   📊 Risk Threshold: {config.heart_attack_threshold}")
    print(f"   🎯 Features: {len(config.heart_attack_features)} features")
    print()
    print("✅ Enhanced Health Agent with Heart Attack Prediction ready!")
    print("🚀 Run Streamlit to start the complete health analysis workflow")
    
    return "Enhanced Agent with Heart Attack Prediction ready for Streamlit integration"
