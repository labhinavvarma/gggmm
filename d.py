...
class HealthDataProcessor:
    ...
    def extract_medical_fields_without_llm(self, deidentified_medical: Dict[str, Any]) -> Dict[str, Any]:
        """Enhanced medical field extraction with date extraction (no LLM calls)"""
        extraction_result = {
            "hlth_srvc_records": [],
            "extraction_summary": {
                "total_hlth_srvc_records": 0,
        ...
        try:
            logger.info("🔍 Starting medical field extraction with dates (no LLM)...")
            ...
            # Enhanced recursive extraction from the entire JSON structure
            self._recursive_medical_extraction_without_llm(medical_data, extraction_result)
    ...
    def _recursive_medical_extraction_without_llm(self, data: Any, result: Dict[str, Any], path: str = ""):
        """Enhanced recursive search with date extraction (no LLM calls)"""
        if isinstance(data, dict):
            current_record = {}
            ...
            # Extract claim received date - CORRECTED FIELD NAME
            date_fields = ['clm_rcvd_st', 'CLM_RCVD_ST', 'claim_received_date', 'received_date']
            for field in date_fields:
                if field in data and data[field]:
                    current_record["claim_received_date"] = str(data[field])
                    result["extraction_summary"]["dates_extracted"] += 1
            ...
            for key, value in data.items():
                new_path = f"{path}.{key}" if path else key
                self._recursive_medical_extraction_without_llm(value, result, new_path)
                
        elif isinstance(data, list):
            for i, item in enumerate(data):
                new_path = f"{path}[{i}]" if path else f"[{i}]"
                self._recursive_medical_extraction_without_llm(item, result, new_path)
    
    def extract_pharmacy_fields_without_llm_from_raw(self, raw_pharmacy: Dict[str, Any]) -> Dict[str, Any]:
        """Enhanced pharmacy field extraction with date extraction from raw data (no LLM calls)"""
        extraction_result = {
            "ndc_records": [],
            "extraction_summary": {
                "total_ndc_records": 0,
        ...
        try:
            logger.info("🔍 Starting pharmacy field extraction with dates from raw data (no LLM)...")
            ...
            self._recursive_pharmacy_extraction_without_llm(pharmacy_data, extraction_result)
            ...
            logger.info(f"💊 Pharmacy extraction from raw data completed: "
                       f"{extraction_result['extraction_summary']['total_ndc_records']} NDC records, "
                       f"{extraction_result['extraction_summary']['dates_extracted']} dates extracted")
            
        except Exception as e:
            logger.error(f"Error in pharmacy field extraction from raw data: {e}")
            extraction_result["error"] = f"Pharmacy extraction from raw data failed: {str(e)}"
        
        return extraction_result
    
    def _recursive_pharmacy_extraction_without_llm(self, data: Any, result: Dict[str, Any], path: str = ""):
        """Enhanced recursive search for pharmacy fields with date extraction (no LLM calls)"""
        if isinstance(data, dict):
            current_record = {}
            ...
            # Extract prescription filled date - CORRECTED FIELD NAME
            ...
            for key, value in data.items():
                new_path = f"{path}.{key}" if path else key
                self._recursive_pharmacy_extraction_without_llm(value, result, new_path)
                
        elif isinstance(data, list):
            for i, item in enumerate(data):
                new_path = f"{path}[{i}]" if path else f"[{i}]"
                self._recursive_pharmacy_extraction_without_llm(item, result, new_path)
