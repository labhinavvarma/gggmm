...
st.set_page_config(
    page_title="Health Agent with MCP Heart Attack Prediction",
    page_icon="🏥",
    layout="wide",
...
# Import the Enhanced LangGraph health analysis agent with MCP Heart Attack Prediction
...
# Custom CSS for Enhanced LangGraph + Snowflake + Chatbot + MCP Heart Attack Prediction themed styling
st.markdown("""
<style>
.main-header {
    font-size: 2.8rem;
...
st.markdown('<h1 class="main-header">🏥 Health Agent with MCP Heart Attack Prediction</h1>', unsafe_allow_html=True)
st.markdown("**Advanced health analysis with comprehensive medical data extraction, interactive chatbot, and MCP-powered heart attack risk prediction**")

# Display import status AFTER page config
if AGENT_AVAILABLE:
    st.success("✅ Health Agent with MCP Heart Attack Prediction imported successfully!")
else:
    ...
    with st.expander("🔧 Health Agent Installation Guide"):
        ...
        **Required Files:**
        ...
        - `MLHDmcpserver.py` (MCP Server)
        - `heart_disease_model_package.pkl` (trained ML model for MCP server)
        
        **MCP Server Setup:**
        1. Start the MCP server: `python MLHDmcpserver.py`
        ...
        **Health Agent Features:**
        ...
        - ✅ **MCP Heart Attack Risk Prediction using AdaBoost model**
        - ✅ **Interactive chatbot with medical data context**
        """)
    st.stop()

# Enhanced Sidebar Configuration with MCP Heart Attack Prediction
with st.sidebar:
    ...
    st.markdown("❤️ **MCP Heart Attack Prediction:** Active")  # NEW
    ...
    # NEW: MCP Heart Attack Prediction Configuration
    st.subheader("❤️ MCP Heart Attack Prediction Settings")
    st.markdown('<div class="mcp-badge">🔗 MCP Server Integration</div>', unsafe_allow_html=True)
    
    mcp_server_url = st.text_input(
        "MCP Server URL *", 
        value="http://localhost:8000",
        help="URL of the MCP server running the heart attack prediction model"
    )
    ...
    st.markdown("**🧪 MCP Server Connection:**")
    if st.button("🔍 Test MCP Connection", key="test_mcp"):
        try:
            # Test MCP connection with a simple request
            import requests
            test_url = f"{mcp_server_url}/tools/call"
            test_payload = {
            ...
            with st.spinner("Testing MCP server connection..."):
                ...
                if response.status_code == 200:
                    ...
                    if "content" in result and result["content"]:
                        prediction = result["content"][0].get("text", "")
                        st.info(f"📊 Test prediction: {prediction}")
                else:
    ...
    with st.expander("🔧 MCP Server Information"):
        st.markdown("""
        **MCP Heart Attack Prediction Model:**
        - **Model Type:** AdaBoost Classifier
        - **Features:** Age, Gender, Diabetes, High_BP, Smoking (5 features)
        - **Input Format:** Binary features (0/1) except Age
        ...
        **Server Requirements:**
        ...
        - FastMCP and joblib libraries
        - Server runs on port 8000 by default
        """)
        
        # Show current MCP URL
        st.code(f"Current MCP URL: {mcp_server_url}")
    ...
    try:
        ...
        st.markdown("**🔧 FastAPI URL and MCP Heart Attack Prediction settings can be modified. API settings are pre-configured.**")
    except Exception as e:
    ...
    # Update configuration with MCP heart attack prediction
    if st.button("🔄 Update Configuration"):
        try:
            ...
            st.success("✅ Configuration updated including MCP heart attack prediction!")
    ...
    # Current Configuration Status with MCP Heart Attack Prediction
    ...
    try:
        ...
        # NEW: Show MCP Heart Attack Prediction settings
        st.markdown("**❤️ MCP Heart Attack Prediction:**")
        st.write(f"**MCP Server URL:** {current_config.mcp_server_url}")
        ...
        st.success("❤️ MCP Heart Attack Prediction is configured!")
        ...
        if st.button("🧪 Test MCP from Agent"):
            try:
                if st.session_state.agent is None:
                    test_config = Config(mcp_server_url=mcp_server_url)
                    test_agent = HealthAnalysisAgent(test_config)
                else:
                    test_agent = st.session_state.agent
                
                with st.spinner("Testing MCP connection from agent..."):
                    # Run async test in sync context
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    mcp_result = loop.run_until_complete(test_agent.test_mcp_connection())
                    loop.close()
                
                if mcp_result["success"]:
                    st.success("✅ MCP connection from agent successful!")
                    st.info(f"📝 Response: {mcp_result['response']}")
                    st.info(f"🔗 MCP Server: {mcp_result['mcp_server_url']}")
                else:
                    st.error("❌ MCP connection from agent failed!")
...
st.info("💡 Enter patient information below. This data will be processed through the Health Agent workflow with AI analysis and MCP heart attack risk prediction using AdaBoost model.")
...
with st.form("patient_input_form"):
    ...
    submitted = st.form_submit_button(
        "🚀 Execute Health Agent Analysis with MCP Heart Attack Prediction", 
...
if st.session_state.analysis_running:
    st.markdown('<div class="info-box">🔄 Health Agent workflow with MCP heart attack prediction executing... Please wait.</div>', unsafe_allow_html=True)

# Run Enhanced LangGraph Analysis with MCP Heart Attack Prediction
if submitted and not st.session_state.analysis_running:
    ...
    st.info(f"📤 Sending patient data to Health Agent with MCP Heart Attack Prediction: {patient_data['first_name']} {patient_data['last_name']} (Age: {calculated_age})")
    ...
    if not is_valid:
        ...
        # Initialize Health Agent with MCP heart attack prediction
        if st.session_state.agent is None:
            try:
                config = st.session_state.config or Config()
                st.session_state.agent = HealthAnalysisAgent(config)
                st.success(f"✅ Health Agent initialized with MCP heart attack prediction")
                st.info(f"🤖 Model: {config.model}")
                st.info(f"🔑 App ID: {config.app_id}")
                st.info(f"❤️ MCP Server: {config.mcp_server_url}")
        ...
        with st.spinner("🚀 Executing Enhanced Health Agent workflow with MCP Heart Attack Prediction..."):
            try:
                ...
                status_text.text("❤️ Node 8: MCP Heart Attack Risk Prediction...")  # NEW
                ...
                if results.get("success", False):
                    ...
                    status_text.text("✅ Enhanced Health Agent workflow with MCP Heart Attack Prediction completed successfully!")
                    
                    st.session_state.analysis_results = results
                    st.session_state.chatbot_context = results.get("chatbot_context", {})
                    st.markdown('<div class="success-box">✅ Enhanced Health Agent analysis with MCP Heart Attack Prediction completed successfully!</div>', unsafe_allow_html=True)
                    ...
                    if heart_attack_prediction and not heart_attack_prediction.get("error"):
                        ...
                        model_source = heart_attack_prediction.get("model_info", {}).get("model_source", "mcp_server")
                        mcp_url = heart_attack_prediction.get("model_info", {}).get("mcp_server_url", "unknown")
                        
                        # Display risk with appropriate styling
                        if risk_level == "HIGH":
                            st.markdown(f'<div class="risk-high">{risk_icon} <strong>MCP Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>⚠️ Immediate medical consultation recommended</div>', unsafe_allow_html=True)
                        elif risk_level == "MODERATE":
                            st.markdown(f'<div class="risk-moderate">{risk_icon} <strong>MCP Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>📋 Regular monitoring advised</div>', unsafe_allow_html=True)
                        elif risk_level == "LOW":
                            st.markdown(f'<div class="risk-low">{risk_icon} <strong>MCP Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>✅ Continue healthy lifestyle practices</div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="mcp-box">{risk_icon} <strong>MCP Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}</div>', unsafe_allow_html=True)
                    elif heart_attack_prediction and heart_attack_prediction.get("error"):
                        st.warning(f"⚠️ MCP Heart Attack Prediction: {heart_attack_prediction.get('error', 'Unknown error')}")
                    
                    if results.get("chatbot_ready", False):
                        st.markdown('<div class="chatbot-box">💬 Interactive chatbot is ready with MCP heart attack prediction context!</div>', unsafe_allow_html=True)
                else:
                ...
                st.session_state.analysis_results = {
                    ...
                    "langgraph_used": True,
                    "enhancement_version": "v4.0_with_mcp_heart_attack_prediction"
                }
            finally:
                st.session_state.analysis_running = False

# Display Health Agent Results with MCP Heart Attack Prediction
if st.session_state.analysis_results:
    ...
    if processed_patient:
        ...
        st.info(f"📋 Enhanced analysis with MCP heart attack prediction completed for: {processed_patient.get('first_name', 'Unknown')} {processed_patient.get('last_name', 'Unknown')}{age_display}")
    ...
    # NEW: MCP Heart Attack Prediction Section (Node 8)
    ...
    if heart_attack_prediction:
        st.markdown('<div class="mcp-header">❤️ Node 8: MCP Heart Attack Risk Prediction</div>', unsafe_allow_html=True)
        
        # Show MCP model badge
        st.markdown('<div class="mcp-box">🔗 <strong>Using MCP Server</strong> - AdaBoost model via MCP protocol</div>', unsafe_allow_html=True)
        ...
        if not heart_attack_prediction.get('error'):
            ...
            # Main risk display with MCP model info
            col1, col2, col3, col4 = st.columns(4)
            ...
            with col4:
                ...
                <div class="entity-card" style="border-color: {risk_color}; background: linear-gradient(135deg, #fff, #f8f9fa);">
                    ...
                    <p class="metric-highlight" style="color: {risk_color}; font-size: 1.2rem;">{model_info.get('model_source', 'mcp_server').upper()}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            if prediction_interpretation:
                st.markdown("**🔍 MCP Model Risk Assessment Interpretation:**")
            ...
            # Feature Analysis for MCP Model
            if heart_attack_features and not heart_attack_features.get('error'):
                with st.expander("🔍 View MCP Model Features Analysis"):
                    ...
                    with col1:
                        st.markdown("**📊 Extracted Features (MCP Model):**")
                    ...
                    if model_info_features:
                        st.markdown("**🤖 MCP Model Information:**")
                        ...
                        st.markdown(f"- **MCP Server URL:** {model_info_features.get('mcp_server_url', 'Unknown')}")
            
            # MCP Model Information
            if model_info:
                with st.expander("🤖 MCP Model Information"):
                    st.json(model_info)
            
            # Show prediction raw text
            prediction_text = heart_attack_prediction.get("prediction_text", "")
            if prediction_text:
                st.markdown("**📄 Raw MCP Prediction Response:**")
                st.code(prediction_text)
            
            # Download MCP heart attack prediction
            st.download_button(
                "📄 Download MCP Heart Attack Prediction",
                safe_json_dumps(heart_attack_prediction),
                f"mcp_heart_attack_prediction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json"
            )
            ...
            st.error(f"❌ MCP Heart Attack Prediction Error: {error_msg}")
            
            model_info = heart_attack_prediction.get('model_info', {})
            if model_info and 'mcp_server_url' in model_info:
                st.warning(f"⚠️ MCP Server: {model_info['mcp_server_url']}")
            
            st.info("💡 MCP heart attack prediction requires a running MCP server. Please check the configuration in the sidebar.")

    # Interactive Chatbot Section (updated to include MCP heart attack prediction context)
    if results.get("chatbot_ready", False) and st.session_state.chatbot_context:
        st.markdown('<div class="chatbot-header">💬 Node 9: Interactive Chatbot with MCP Heart Attack Prediction</div>', unsafe_allow_html=True)
        
        st.markdown('<div class="chatbot-box">🤖 Ask questions about the patient\'s medical data, analysis results, MCP heart attack risk assessment, or request specific insights based on the deidentified records.</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            context_summary = {
                ...
                "MCP Heart Attack Prediction": "✅ Available" if st.session_state.chatbot_context.get("heart_attack_prediction") else "❌ None"  # NEW
            }
        ...
        # Chat input
        user_question = st.chat_input("💬 Ask a question about the medical data or MCP heart attack risk...")
        ...
        if user_question:
            ...
            try:
                with st.spinner("🤖 Analyzing medical data and MCP heart attack prediction, generating response..."):
        ...
        # Enhanced Sample questions including MCP heart attack prediction
        st.markdown("**💡 Sample Questions You Can Ask:**")
        sample_questions = [
            "What medications was this patient prescribed?",
            "Are there any chronic conditions indicated in the medical data?",
            "What is the patient's MCP heart attack risk assessment?",  # NEW
            "What factors contribute to the MCP heart attack risk score?",  # NEW
            "Explain the significance of the ICD-10 diagnosis codes found",
            "What drug interactions should be considered?",
            "How do the medications relate to the MCP heart attack risk prediction?",  # NEW
            "Summarize the key health insights including MCP heart attack risk"  # NEW
        ]
    ...
    # Complete Health Agent Analysis Report Download (updated with MCP heart attack prediction)
    st.markdown('<div class="step-header">💾 Complete Health Agent Analysis Report</div>', unsafe_allow_html=True)
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        # Complete JSON report with MCP heart attack prediction
        complete_report = {
            "health_agent_metadata": {
                ...
                "mcp_heart_attack_prediction_enabled": True,  # NEW
                ...
                "interactive_features": ["Medical Data Q&A", "ICD-10 Analysis", "NDC Code Interpretation", "MCP Heart Attack Risk Assessment"]  # NEW
            },
            "api_outputs": safe_get(results, "api_outputs", {}),
            "deidentified_data": safe_get(results, "deidentified_data", {}),
            "structured_extractions": safe_get(results, "structured_extractions", {}),
            ...
            "health_trajectory": safe_get(results, "health_trajectory", ""),
            "final_summary": safe_get(results, "final_summary", ""),
            # NEW: MCP heart attack prediction data
            "mcp_heart_attack_prediction": safe_get(results, "heart_attack_prediction", {}),
            "heart_attack_risk_score": safe_get(results, "heart_attack_risk_score", 0.0),
            "heart_attack_features": safe_get(results, "heart_attack_features", {}),
        ...
        st.download_button(
            ...
            f"health_agent_analysis_mcp_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            mime="application/json",
            use_container_width=True
        )
    
    with col2:
        # Text report with MCP heart attack prediction info
        ...
        # MCP heart attack prediction info
        ...
        model_source = safe_get(heart_attack_prediction, "model_info", {}).get("model_source", "mcp_server")
        mcp_server_url = safe_get(heart_attack_prediction, "model_info", {}).get("mcp_server_url", "unknown")
        
        text_report = f"""
HEALTH AGENT ANALYSIS REPORT WITH MCP HEART ATTACK PREDICTION
===========================================================
...
MCP Heart Attack Prediction: {'Enabled' if heart_attack_prediction else 'Disabled'}
Enhancement Version: {results.get('enhancement_version', 'v4.0')}

MCP HEART ATTACK RISK ASSESSMENT:
===============================
...
MCP HEART ATTACK PREDICTION DETAILS:
===================================
{safe_json_dumps(heart_attack_prediction)}
...
{chr(10).join(safe_get(results, 'errors', []))}
        ...
        st.download_button(
            ...
            text_report,
            f"health_agent_analysis_mcp_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
            mime="text/plain",
            use_container_width=True
        )
    
    with col3:
        # CSV summary with MCP heart attack prediction metrics
        try:
            # Get extraction counts
            medical_extraction = safe_get(structured_extractions, 'medical', {})
            pharmacy_extraction = safe_get(structured_extractions, 'pharmacy', {})
            ...
            # MCP heart attack prediction metrics
            heart_risk_score = results.get("heart_attack_risk_score", 0.0)
            heart_risk_level = safe_get(heart_attack_prediction, "risk_level", "Unknown")
            model_source = safe_get(heart_attack_prediction, "model_info", {}).get("model_source", "mcp_server")
            mcp_server_url = safe_get(heart_attack_prediction, "model_info", {}).get("mcp_server_url", "unknown")
            ...
            csv_data = {
                "Metric": [
                    ...
                    "MCP Heart Attack Risk Score", "MCP Heart Attack Risk Level", "Model Source", "MCP Server URL", "Chatbot Ready", "Chat Messages", "Timestamp"  # NEW
                ],
                "Value": [
            ...
            st.download_button(
                ...
                csv_string,
                f"health_agent_summary_mcp_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                use_container_width=True
            )
        except Exception as e:
...
<div style='text-align: center; color: #666;'>
    🏥 <strong>Health Agent Dashboard with MCP Heart Attack Prediction</strong><br>
    Powered by Advanced AI with Medical/Pharmacy Extraction, MCP ML Heart Attack Risk Assessment, and Interactive Medical Data Chatbot<br>
    ✅ <strong>Features:</strong> Medical ICD-10 Code Extraction | Pharmacy NDC Data Extraction | AI Analysis | MCP Heart Attack Risk Prediction | Interactive Medical Data Q&A Chatbot<br>
    🔗 <strong>MCP Integration:</strong> AdaBoost Model via MCP Server (Age, Gender, Diabetes, High_BP, Smoking)<br>
...
if st.sidebar.checkbox("🐛 Show Debug Info"):
    ...
    if st.session_state.config:
        ...
        # NEW: MCP heart attack prediction debug
        st.sidebar.write("MCP Server URL:", st.session_state.config.mcp_server_url)
        st.sidebar.write("Heart Attack Threshold:", st.session_state.config.heart_attack_threshold)
        
        # Try to show MCP server status
        try:
            import requests
            test_url = f"{st.session_state.config.mcp_server_url}/tools/call"
            response = requests.get(st.session_state.config.mcp_server_url.replace("/tools/call", ""), timeout=5)
            st.sidebar.write("MCP Server Status:", "✅ Reachable" if response.status_code < 500 else "❌ Error")
        except Exception as e:
            st.sidebar.write("MCP Server Status:", f"❌ {str(e)[:50]}")
    else:
...
if st.sidebar.checkbox("📚 Show Usage Instructions"):
    ...
    **❤️ MCP Heart Attack Prediction:**
    - Uses AdaBoost model via MCP server
    ...
    **🔗 MCP Server Setup:**
    1. Start MCP server: `python MLHDmcpserver.py`
    2. Server runs on http://localhost:8000
...
# MCP Server Information Panel
if st.sidebar.checkbox("🔗 Show MCP Server Information"):
    st.sidebar.markdown("### 🔗 MCP Server Information")
    
    try:
        if st.session_state.config:
            mcp_url = st.session_state.config.mcp_server_url
            st.sidebar.success("✅ MCP Configuration Available")
            st.sidebar.write(f"**URL:** {mcp_url}")
            
            # Test MCP server connectivity
            try:
                import requests
                response = requests.get(mcp_url.replace("/tools/call", ""), timeout=5)
                if response.status_code < 500:
                    st.sidebar.success("✅ MCP Server Reachable")
                else:
                    st.sidebar.error(f"❌ MCP Server Error: {response.status_code}")
            except Exception as e:
                st.sidebar.error(f"❌ MCP Connection Failed: {str(e)}")
            ...
            st.sidebar.write("**Protocol:** MCP (Model Context Protocol)")
            st.sidebar.write("**Output:** Risk probability + level")
        else:
            st.sidebar.warning("⚠️ No configuration available")
    except Exception as e:
        st.sidebar.error(f"❌ MCP info error: {str(e)}")
...
if st.sidebar.checkbox("📈 Show Performance Metrics"):
    ...
    if st.session_state.analysis_results:
        ...
        if results.get('heart_attack_prediction'):
            ...
            st.sidebar.metric("MCP Heart Attack Risk Score", f"{risk_score:.3f}")
