...
class HealthAnalysisAgent:
    ...
    def _load_combined_heart_attack_model(self):
        """Load the combined heart attack prediction model from PKL file"""
        try:
            ...
            # Handle different possible structures of the combined PKL
            if isinstance(combined_data, dict):
                # If it's a dictionary with separate components
                self.heart_attack_model = combined_data.get('model')
                self.heart_attack_scaler = combined_data.get('scaler')
                self.heart_attack_features = combined_data.get('feature_names', ['Age', 'Gender', 'Diabetes', 'High_BP', 'Smoking'])
                
                logger.info("✅ Combined heart attack prediction model loaded from dictionary structure")
                
            elif hasattr(combined_data, 'model') and hasattr(combined_data, 'scaler'):
                # If it's an object with model and scaler attributes
                self.heart_attack_model = combined_data.model
                self.heart_attack_scaler = combined_data.scaler
                # Try to get feature names from different possible attributes
                if hasattr(combined_data, 'feature_names'):
                    self.heart_attack_features = combined_data.feature_names
                elif hasattr(combined_data, 'feature_names_in_'):
                    self.heart_attack_features = combined_data.feature_names_in_
                else:
            ...
            elif isinstance(combined_data, tuple) or isinstance(combined_data, list):
                # If it's a tuple/list with (model, scaler, features)
                self.heart_attack_model = combined_data[0] if len(combined_data) > 0 else None
                self.heart_attack_scaler = combined_data[1] if len(combined_data) > 1 else None
                self.heart_attack_features = combined_data[2] if len(combined_data) > 2 else ['Age', 'Gender', 'Diabetes', 'High_BP', 'Smoking']
                
                logger.info("✅ Combined heart attack prediction model loaded from tuple/list structure")
                
            else:
                # If it's just the model itself, assume no scaler
                self.heart_attack_model = combined_data
                self.heart_attack_scaler = None
                self.heart_attack_features = ['Age', 'Gender', 'Diabetes', 'High_BP', 'Smoking']
                
                logger.info("✅ Heart attack prediction model loaded (no scaler)")
            
            # Validate that we have at least the model
            if self.heart_attack_model is None:
                logger.error("❌ No valid model found in combined PKL file")
                return
