...
class HealthAnalysisAgent:
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Enhanced chatbot with COMPLETE deidentified data access - NO truncation"""
        try:
            # Use enhanced data processor to prepare COMPLETE context - ENTIRE JSON data
            ...
            # Create COMPLETE prompt with ENTIRE deidentified medical data - NO TRUNCATION
            complete_prompt = f"""You are an expert medical data assistant with access to the COMPLETE, ENTIRE deidentified patient health records. You have the FULL JSON data structures available. Answer the user's question with specific, detailed information from ANY part of the complete medical data provided.

COMPLETE DEIDENTIFIED PATIENT DATA (ENTIRE JSON STRUCTURES - NO TRUNCATION):
{complete_context}
...
- You have access to the COMPLETE deidentified medical and pharmacy JSON data
...
DETAILED ANSWER USING COMPLETE DEIDENTIFIED DATA:"""

            logger.info(f"💬 Processing query with COMPLETE deidentified data access: {user_query[:50]}...")
            ...
            enhanced_system_msg = """You are a powerful healthcare AI assistant with access to COMPLETE deidentified medical records. You can search through and reference ANY part of the provided JSON data structures. Provide accurate, detailed analysis based on the ENTIRE medical and pharmacy data provided. Always maintain patient privacy and provide professional medical insights using the complete available data."""
            
            response = self.api_integrator.call_llm(complete_prompt, enhanced_system_msg)
            
            if response.startswith("Error"):
                return "I encountered an error processing your question. Please try rephrasing your question. I have access to the complete deidentified medical and pharmacy data and can answer questions about any specific codes, medications, dates, or other data points."
            
            return response
            
        except Exception as e:
            logger.error(f"Error in complete deidentified data chatbot: {str(e)}")
            return "I encountered an error processing your question. Please try again. I have access to the complete deidentified medical and pharmacy JSON data and can answer detailed questions about any aspect of the patient's records."
