...
✅ **MCP Server Results:**
- API Calls: {len([k for k, v in result.get('raw_api_responses', {}).items() if v and not v.get('error')])}/5 successful
...
🎯 **Entity Extraction:**
- Diabetes: {result.get('entity_extraction', {}).get('diabetes', 'unknown').title()}
- Age Group: {result.get('entity_extraction', {}).get('age_group', 'unknown').title()}
- Blood Pressure: {result.get('entity_extraction', {}).get('blood_pressure', 'unknown').title()}

💬 **JSON data is now available below the chat. You can ask me detailed questions about the deidentified medical and pharmacy data!**

Try asking:
- "What specific medications were found?"
- "Show me the medical conditions in detail"
- "What does the pharmacy data reveal?"
...
I can analyze patient healthcare data through natural language commands and then answer detailed questions about the results.

**🔄 How it works:**
1. **Give me a command** with patient data
2. **I'll call MCP server** and get all JSON responses  
3. **Data gets deidentified** for privacy
4. **You can ask questions** about the deidentified medical and pharmacy data

**📝 Example analysis commands:**
- "Analyze patient John Smith, DOB 1980-01-15, male, SSN 123456789, zip code 12345"
- "Evaluate health data for Sarah Johnson, born 1975-08-22, female, SSN 987654321, zip 90210"
- "Check patient records for Michael Brown, DOB 1990-12-05, male, SSN 111223333, zip 77001"

**💬 After analysis, ask me questions like:**
- "What medications were found?"
- "Explain the diabetes findings in detail"
- "What medical conditions are identified in the JSON data?"
- "Show me specific values from the pharmacy response"
- "What does the deidentified medical data reveal?"

**🔄 Use the Refresh button (next to Send) to start a new patient analysis.**
...
with col3:
    if st.button("🔄 Refresh", key="refresh_button", help="Reset conversation and analysis"):
        refresh_everything()
...
with st.expander("💡 Example Commands & Quick Actions", expanded=False):
    ...
    **Follow-up Questions (after analysis):**
    ...
    # Quick command buttons inside expander
