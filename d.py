...
# Custom CSS for clean layout with code explanations
st.markdown("""
<style>
.main-header {
    font-size: 2.5rem;
...
def display_enhanced_medical_extraction(structured_extractions, agent):
    """Display medical extraction with code explanations"""
...
def display_enhanced_pharmacy_extraction(structured_extractions, agent):
    """Display pharmacy extraction with code explanations"""
    ...
    if st.session_state.analysis_results and st.session_state.analysis_results.get("chatbot_ready", False) and st.session_state.chatbot_context:
        ...
        with chat_container:
            if st.session_state.chatbot_messages:
                ...
                st.info("💡 **Ask about codes:** I can explain medical codes, NDC numbers, and medication details!")
        ...
        st.markdown("• Answer questions about medical data")
        st.markdown("• Analyze diagnoses and medications") 
        st.markdown("• Explain medical codes and NDC numbers")
            ...
            try:
                ...
                if results.get("success", False):
                    ...
                    if results.get("chatbot_ready", False) and st.session_state.chatbot_context:
                        st.success("💬 Medical Assistant is now available in the sidebar with full access to all medical data!")
...
if st.session_state.analysis_results:
    ...
    # 4. MEDICAL/PHARMACY DATA EXTRACTION BUTTON WITH LLM CODE EXPLANATIONS
    if st.button("🔍 Medical/Pharmacy Data Extraction", use_container_width=True):
        ...
        if structured_extractions:
            ...
            with tab1:
                st.markdown("### Medical Claims Extraction with LLM Explanations")
            ...
            with tab2:
                st.markdown("### Pharmacy Claims Extraction with LLM Explanations")
    ...
    if st.button("❤️ Heart Attack Risk Prediction", use_container_width=True):
        ...
        if heart_attack_prediction and not heart_attack_prediction.get('error'):
            ...
            st.info(f"💡 Expected FastAPI Server: {st.session_state.config.heart_attack_api_url if st.session_state.config else 'http://localhost:8002'}")
            st.info("💡 Make sure FastAPI server is running: `python heart_attack_fastapi_server.py`")

# Footer
st.markdown("---")
st.markdown("### 🔬 Enhanced Health Analysis Agent")
st.markdown("**Features:** LLM Code Explanations • Claims Data Processing • Heart Attack Prediction • AI Medical Assistant")
st.markdown("*Built with LangGraph, Streamlit, and Snowflake Cortex*")
