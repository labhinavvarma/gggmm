...
if AGENT_AVAILABLE:
    ...
    with st.expander("🔧 Health Agent Installation Guide"):
        ...
        **Health Agent Features:**
        ...
        - ✅ **Heart Attack Risk Prediction using AdaBoost model**
...
st.info("💡 Enter patient information below. This data will be processed through the Health Agent workflow with AI analysis and heart attack risk prediction using AdaBoost model.")
    ...
    if not is_valid:
        ...
        with st.spinner("🚀 Executing Health Agent workflow..."):
            try:
                ...
                status_text.text("❤️ Node 8: Heart Attack Risk Prediction...")
                ...
                if results.get("success", False):
                    ...
                    if heart_attack_prediction and not heart_attack_prediction.get("error"):
                        ...
                        if risk_level == "HIGH":
                            st.markdown(f'<div class="risk-high">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>⚠️ Immediate medical consultation recommended</div>', unsafe_allow_html=True)
                        elif risk_level == "MODERATE":
                            st.markdown(f'<div class="risk-moderate">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>📋 Regular monitoring advised</div>', unsafe_allow_html=True)
                        elif risk_level == "LOW":
                            st.markdown(f'<div class="risk-low">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}<br>✅ Continue healthy lifestyle practices</div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="mcp-box">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>Model Source: {model_source}<br>MCP Server: {mcp_url}</div>', unsafe_allow_html=True)
...
if st.session_state.analysis_results:
    ...
    if heart_attack_prediction:
        st.markdown('<div class="mcp-header">❤️ Node 8: Heart Attack Risk Prediction</div>', unsafe_allow_html=True)
        ...
        if not heart_attack_prediction.get('error'):
            ...
            if prediction_text:
                st.markdown("**📄 Raw Prediction Response:**")
            ...
            st.info("💡 Heart attack prediction requires a running MCP server. Please check the configuration in the sidebar.")
        ...
        try:
            ...
            csv_data = {
                "Metric": [
                    ...
                    "Heart Attack Risk Score", "Heart Attack Risk Level", "Model Source", "MCP Server URL", "Chatbot Ready", "Chat Messages", "Timestamp"
                ],
                "Value": [
...
<div style='text-align: center; color: #666;'>
    ...
    Powered by Advanced AI with Medical/Pharmacy Extraction, Heart Attack Risk Assessment, and Interactive Medical Data Chatbot<br>
