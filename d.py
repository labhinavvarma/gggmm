...
def display_api_responses_collapsed(api_responses: Dict[str, Any]):
    """Display API responses in collapsed format at the top"""
    ...
    api_tabs = st.tabs(["🆔 MCID", "🏥 Medical", "🔑 Token", "💊 Pharmacy"])
    
    with api_tabs[0]:  # MCID
        mcid_data = api_responses.get('mcid', {})
        if mcid_data:
            with st.expander("🔍 MCID Search Results", expanded=False):
                st.json(mcid_data)
                if mcid_data.get('status_code') == 200:
                    st.success(f"✅ MCID API Success - Status: {mcid_data.get('status_code')}")
                else:
                    st.error(f"❌ MCID API Error - Status: {mcid_data.get('status_code')}")
        else:
            st.info("No MCID data available")
    
    with api_tabs[1]:  # Medical
        medical_data = api_responses.get('medical', {})
        if medical_data:
            with st.expander("🔍 Medical API Results", expanded=False):
                st.json(medical_data)
                if medical_data.get('status_code') == 200:
                    st.success(f"✅ Medical API Success - Status: {medical_data.get('status_code')}")
                else:
                    st.error(f"❌ Medical API Error - Status: {medical_data.get('status_code')}")
        else:
            st.info("No medical data available")
    
    with api_tabs[2]:  # Token
        token_data = api_responses.get('token', {})
        if token_data:
            with st.expander("🔍 Token API Results", expanded=False):
                st.json(token_data)
                if token_data.get('status_code') == 200:
                    st.success(f"✅ Token API Success - Status: {token_data.get('status_code')}")
                else:
                    st.error(f"❌ Token API Error - Status: {token_data.get('status_code')}")
        else:
            st.info("No token data available")
    
    with api_tabs[3]:  # Pharmacy
        pharmacy_data = api_responses.get('pharmacy', {})
        if pharmacy_data:
            with st.expander("🔍 Pharmacy API Results", expanded=False):
                st.json(pharmacy_data)
                if pharmacy_data.get('status_code') == 200:
                    st.success(f"✅ Pharmacy API Success - Status: {pharmacy_data.get('status_code')}")
                else:
                    st.error(f"❌ Pharmacy API Error - Status: {pharmacy_data.get('status_code')}")
        else:
...
for i, message in enumerate(st.session_state.messages):
    ...
    if message["role"] == "assistant" and i < len(st.session_state.analysis_results):
        ...
        if result and result.get('api_responses'):
            # Display raw API responses in collapsed format
...
if prompt := st.chat_input("Ask about healthcare analysis, patient data, or medical information..."):
    ...
    with st.spinner("🤖 Processing your request through Enhanced LangGraph workflow..."):
        try:
            ...
            # Display raw API responses first (at the top, collapsed)
