...
# Import the Enhanced LangGraph health analysis agent with Heart Attack Prediction
...
# Custom CSS for Enhanced LangGraph + Snowflake + Chatbot + Heart Attack Prediction themed styling
st.markdown("""
<style>
.main-header {
    font-size: 2.8rem;
...
st.markdown('<h1 class="main-header">🏥 Health Agent with Heart Attack Prediction</h1>', unsafe_allow_html=True)
st.markdown("**Advanced health analysis with comprehensive medical data extraction, interactive chatbot, and ML-powered heart attack risk prediction**")

# Display import status AFTER page config
if AGENT_AVAILABLE:
    st.success("✅ Health Agent with Heart Attack Prediction imported successfully!")
else:
    ...
    with st.expander("🔧 Health Agent Installation Guide"):
        ...
        **Required Files:**
        ...
        - `heart_attack_model.pkl` (your trained ML model)
        - `heart_attack_scaler.pkl` (optional feature scaler)
        ...
        **Health Agent Features:**
        ...
        - ✅ **Heart Attack Risk Prediction using ML model**
        - ✅ **Interactive chatbot with medical data context**
        """)
    st.stop()

# Enhanced Sidebar Configuration with Heart Attack Prediction
with st.sidebar:
    ...
    st.markdown("❤️ **Heart Attack Prediction:** Active")  # NEW
    ...
    # NEW: Heart Attack Prediction Configuration
    st.subheader("❤️ Heart Attack Prediction Settings")
    heart_attack_pkl_path = st.text_input(
        "Heart Attack Model PKL Path *", 
        value="heart_attack_model.pkl",
        help="Path to the trained heart attack prediction model (PKL file)"
    )
    
    heart_attack_scaler_path = st.text_input(
        "Scaler PKL Path (Optional)", 
        value="heart_attack_scaler.pkl",
        help="Path to the feature scaler (PKL file) - optional"
    )
    ...
    # File upload for PKL files
    st.markdown("**📁 Or Upload PKL Files:**")
    uploaded_model = st.file_uploader(
        "Upload Heart Attack Model", 
        type=['pkl'], 
        key="model_upload",
        help="Upload your trained heart attack prediction model"
    )
    
    uploaded_scaler = st.file_uploader(
        "Upload Feature Scaler", 
        type=['pkl'], 
        key="scaler_upload",
        help="Upload feature scaler (optional)"
    )
    
    # Model info display
    if os.path.exists(heart_attack_pkl_path):
        st.success(f"✅ Model file found: {heart_attack_pkl_path}")
    else:
        st.warning(f"⚠️ Model file not found: {heart_attack_pkl_path}")
    
    if os.path.exists(heart_attack_scaler_path):
        st.success(f"✅ Scaler file found: {heart_attack_scaler_path}")
    else:
        st.info(f"ℹ️ Scaler file not found: {heart_attack_scaler_path}")
    ...
    # Update configuration with heart attack prediction
    if st.button("🔄 Update Configuration"):
        try:
            # Handle uploaded files
            if uploaded_model:
                # Save uploaded model temporarily
                model_path = f"temp_heart_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
                with open(model_path, "wb") as f:
                    f.write(uploaded_model.read())
                heart_attack_pkl_path = model_path
                st.success(f"✅ Model uploaded and saved as: {model_path}")
            
            if uploaded_scaler:
                # Save uploaded scaler temporarily
                scaler_path = f"temp_heart_scaler_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
                with open(scaler_path, "wb") as f:
                    f.write(uploaded_scaler.read())
                heart_attack_scaler_path = scaler_path
                st.success(f"✅ Scaler uploaded and saved as: {scaler_path}")
            
            config = Config(
                fastapi_url=fastapi_url,
                max_retries=max_retries,
                timeout=timeout,
                heart_attack_pkl_path=heart_attack_pkl_path,
                heart_attack_scaler_path=heart_attack_scaler_path,
                heart_attack_threshold=heart_attack_threshold
            )
            st.session_state.config = config
            st.session_state.agent = None  # Force reinitialization
            st.success("✅ Configuration updated including heart attack prediction!")
    ...
    # Current Configuration Status with Heart Attack Prediction
    ...
    try:
        ...
        # NEW: Show Heart Attack Prediction settings
        st.markdown("**❤️ Heart Attack Prediction:**")
        st.write(f"**Model Path:** {current_config.heart_attack_pkl_path}")
        st.write(f"**Scaler Path:** {current_config.heart_attack_scaler_path}")
        st.write(f"**Risk Threshold:** {current_config.heart_attack_threshold}")
        st.write(f"**Features:** {len(current_config.heart_attack_features)}")
...
st.info("💡 Enter patient information below. This data will be processed through the Health Agent workflow with AI analysis and heart attack risk prediction using your ML model.")
...
with st.form("patient_input_form"):
    ...
    submitted = st.form_submit_button(
        "🚀 Execute Health Agent Analysis with Heart Attack Prediction", 
...
if st.session_state.analysis_running:
    st.markdown('<div class="info-box">🔄 Health Agent workflow with heart attack prediction executing... Please wait.</div>', unsafe_allow_html=True)

# Run Enhanced LangGraph Analysis with Heart Attack Prediction
if submitted and not st.session_state.analysis_running:
    ...
    st.info(f"📤 Sending patient data to Health Agent with Heart Attack Prediction: {patient_data['first_name']} {patient_data['last_name']} (Age: {calculated_age})")
    ...
    if not is_valid:
        ...
        # Initialize Health Agent with heart attack prediction
        if st.session_state.agent is None:
            try:
                config = st.session_state.config or Config()
                st.session_state.agent = HealthAnalysisAgent(config)
                st.success(f"✅ Health Agent initialized with heart attack prediction")
                st.info(f"🤖 Model: {config.model}")
                st.info(f"🔑 App ID: {config.app_id}")
                st.info(f"❤️ Heart Attack Model: {config.heart_attack_pkl_path}")
        ...
        with st.spinner("🚀 Executing Enhanced Health Agent workflow with Heart Attack Prediction..."):
            try:
                ...
                status_text.text("❤️ Node 8: Heart Attack Risk Prediction...")  # NEW
                ...
                if results.get("success", False):
                    ...
                    status_text.text("✅ Enhanced Health Agent workflow with Heart Attack Prediction completed successfully!")
                    
                    st.session_state.analysis_results = results
                    st.session_state.chatbot_context = results.get("chatbot_context", {})
                    st.markdown('<div class="success-box">✅ Enhanced Health Agent analysis with Heart Attack Prediction completed successfully!</div>', unsafe_allow_html=True)
                    ...
                    if heart_attack_prediction and not heart_attack_prediction.get("error"):
                        ...
                        if risk_level == "HIGH":
                            st.markdown(f'<div class="risk-high">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>⚠️ Immediate medical consultation recommended</div>', unsafe_allow_html=True)
                        elif risk_level == "MODERATE":
                            st.markdown(f'<div class="risk-moderate">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>📋 Regular monitoring advised</div>', unsafe_allow_html=True)
                        elif risk_level == "LOW":
                            st.markdown(f'<div class="risk-low">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}<br>✅ Continue healthy lifestyle practices</div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="heart-box">{risk_icon} <strong>Heart Attack Risk Assessment: {risk_level}</strong><br>Risk Score: {risk_percentage}</div>', unsafe_allow_html=True)
                ...
                st.session_state.analysis_results = {
                    ...
                    "langgraph_used": True,
                    "enhancement_version": "v4.0_with_heart_attack_prediction"
                }
            finally:
                st.session_state.analysis_running = False

# Display Health Agent Results with Heart Attack Prediction
if st.session_state.analysis_results:
    ...
    if processed_patient:
        ...
        st.info(f"📋 Enhanced analysis with heart attack prediction completed for: {processed_patient.get('first_name', 'Unknown')} {processed_patient.get('last_name', 'Unknown')}{age_display}")
    ...
    # API Outputs Section (same as before - keeping for brevity)
    ...
    # Step 2: Deidentified Data (keeping structure same as before for brevity)
    ...
    # Step 3: Structured Extractions (keeping same structure for brevity)
    ...
    # Step 4: Enhanced Entity Extraction (keeping same structure for brevity)
    ...
    if entity_extraction:
        ...
        # Show medical conditions and medications if available (keeping same structure)
    ...
    # Step 5: Health Trajectory Analysis (keeping same structure)
    ...
    # Step 6: Final Summary (keeping same structure)
    ...
    # NEW: Heart Attack Prediction Section (Node 8)
    ...
    if heart_attack_prediction:
        st.markdown('<div class="heart-header">❤️ Node 8: Heart Attack Risk Prediction</div>', unsafe_allow_html=True)
        ...
        if not heart_attack_prediction.get('error'):
            ...
            with col3:
                ...
                <div class="entity-card" style="border-color: {risk_color}; background: linear-gradient(135deg, #fff, #f8f9fa);">
                    ...
                    <p class="metric-highlight" style="color: {risk_color}; font-size: 1.2rem;">{heart_attack_prediction.get('model_info', {}).get('model_type', 'Unknown')}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col4:
                st.markdown(f"""
                <div class="entity-card" style="border-color: {risk_color}; background: linear-gradient(135deg, #fff, #f8f9fa);">
                    <h3 style="color: {risk_color};">🎯 Features Used</h3>
                    <p class="metric-highlight" style="color: {risk_color}; font-size: 1.5rem;">{heart_attack_prediction.get('model_info', {}).get('features_used', 0)}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            if heart_attack_features and not heart_attack_features.get('error'):
                with st.expander("🔍 View Heart Attack Prediction Features"):
            ...
            st.info("💡 Heart attack prediction requires a trained ML model (PKL file). Please check the configuration in the sidebar.")

    # Node 9: Interactive Chatbot Section (updated to include heart attack prediction context)
    if results.get("chatbot_ready", False) and st.session_state.chatbot_context:
        st.markdown('<div class="chatbot-header">💬 Node 9: Interactive Chatbot with Heart Attack Prediction</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            context_summary = {
                ...
                "Heart Attack Prediction": "✅ Available" if st.session_state.chatbot_context.get("heart_attack_prediction") else "❌ None"  # NEW
            }
        ...
        # Enhanced Sample questions including heart attack prediction
        st.markdown("**💡 Sample Questions You Can Ask:**")
        sample_questions = [
            "What medications was this patient prescribed?",
            "Are there any chronic conditions indicated in the medical data?",
            "What is the patient's heart attack risk assessment?",  # NEW
            "What factors contribute to the heart attack risk score?",  # NEW
            "Explain the significance of the ICD-10 diagnosis codes found",
            "What drug interactions should be considered?",
            "How do the medications relate to the heart attack risk prediction?",  # NEW
            "Summarize the key health insights including heart attack risk"  # NEW
        ]
    ...
    # Complete Health Agent Analysis Report Download (updated with heart attack prediction)
    ...
    with col1:
        ...
        complete_report = {
            "health_agent_metadata": {
                ...
                "heart_attack_prediction_enabled": True,  # NEW
                ...
                "interactive_features": ["Medical Data Q&A", "ICD-10 Analysis", "NDC Code Interpretation", "Heart Attack Risk Assessment"]  # NEW
            },
            "api_outputs": safe_get(results, "api_outputs", {}),
            "deidentified_data": safe_get(results, "deidentified_data", {}),
            "structured_extractions": safe_get(results, "structured_extractions", {}),
            ...
            "health_trajectory": safe_get(results, "health_trajectory", ""),
            "final_summary": safe_get(results, "final_summary", ""),
            # NEW: Heart attack prediction data
            "heart_attack_prediction": safe_get(results, "heart_attack_prediction", {}),
            "heart_attack_risk_score": safe_get(results, "heart_attack_risk_score", 0.0),
            "heart_attack_features": safe_get(results, "heart_attack_features", {}),
        ...
        st.download_button(
            ...
            f"health_agent_analysis_with_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            mime="application/json",
            use_container_width=True
        )
    
    with col2:
        # Text report with heart attack prediction info
        ...
        text_report = f"""
HEALTH AGENT ANALYSIS REPORT WITH HEART ATTACK PREDICTION
=========================================================
...
{chr(10).join(safe_get(results, 'errors', []))}
        ...
        st.download_button(
            ...
            text_report,
            f"health_agent_analysis_with_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
            mime="text/plain",
            use_container_width=True
        )
    
    with col3:
        # CSV summary with heart attack prediction metrics
        try:
            # Get extraction counts
            medical_extraction = safe_get(structured_extractions, 'medical', {})
            pharmacy_extraction = safe_get(structured_extractions, 'pharmacy', {})
            ...
            csv_data = {
                "Metric": [
                    ...
                    "Heart Attack Risk Score", "Heart Attack Risk Level", "Chatbot Ready", "Chat Messages", "Timestamp"  # NEW
                ],
                "Value": [
            ...
            st.download_button(
                ...
                csv_string,
                f"health_agent_summary_with_heart_prediction_{patient_last_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                use_container_width=True
            )
        except Exception as e:
...
<div style='text-align: center; color: #666;'>
    🏥 <strong>Health Agent Dashboard with Heart Attack Prediction</strong><br>
    Powered by Advanced AI with Medical/Pharmacy Extraction, ML Heart Attack Risk Assessment, and Interactive Medical Data Chatbot<br>
    ✅ <strong>Features:</strong> Medical ICD-10 Code Extraction | Pharmacy NDC Data Extraction | AI Analysis | Heart Attack Risk Prediction | Interactive Medical Data Q&A Chatbot<br>
...
if st.sidebar.checkbox("🐛 Show Debug Info"):
    ...
    if st.session_state.config:
        ...
        # NEW: Heart attack prediction debug
        st.sidebar.write("Heart Attack Model:", st.session_state.config.heart_attack_pkl_path)
        st.sidebar.write("Heart Attack Threshold:", st.session_state.config.heart_attack_
