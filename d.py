...
class HealthAnalysisAgent:
    ...
    def _handle_heart_attack_question(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            # Create enhanced prompt for heart attack analysis (SINGLE LINE)
            heart_attack_prompt = f"You are an expert cardiologist and data analyst with access to COMPLETE deidentified patient claims data. Analyze the heart attack/cardiovascular risk based on the comprehensive medical and pharmacy claims data provided. COMPLETE DEIDENTIFIED CLAIMS DATA FOR HEART ATTACK ANALYSIS: {complete_context} CURRENT FASTAPI ML MODEL PREDICTION: {json.dumps(heart_attack_prediction, indent=2)} EXTRACTED HEALTH ENTITIES: {json.dumps(entity_extraction, indent=2)} RECENT CONVERSATION HISTORY: {history_text} USER QUESTION: {user_query} CRITICAL ANALYSIS INSTRUCTIONS: - Provide a comprehensive heart attack/cardiovascular risk assessment in PERCENTAGE format - Analyze ALL available medical codes (ICD-10), medications (NDC), and claims data - Consider age, gender, diabetes status, blood pressure, smoking, and medication patterns - Compare your analysis with the FastAPI ML model prediction provided above - Provide specific percentages for cardiovascular risk based on the complete claims data - Reference specific medical codes, medications, dates, and clinical indicators from the JSON data - Explain discrepancies between your analysis and the FastAPI model if any - Include risk factors found in the claims data that support your percentage assessment PROVIDE YOUR RESPONSE IN THIS FORMAT: **🤖 LLM CARDIOVASCULAR RISK ANALYSIS:** - **Risk Percentage:** [Your calculated percentage]% - **Risk Category:** [Low/Medium/High Risk] - **Key Risk Factors:** [List specific factors from claims data] - **Supporting Evidence:** [Specific codes, medications, dates from JSON] **⚖️ COMPARISON WITH FASTAPI MODEL:** - **FastAPI Prediction:** [FastAPI percentage and category] - **LLM Analysis:** [Your percentage and category] - **Agreement/Discrepancy:** [Comparison and explanation] - **Confidence:** [Your confidence in the assessment] **📊 DETAILED CARDIOVASCULAR ASSESSMENT:** [Provide detailed analysis based on complete claims data] Use the complete deidentified claims data to provide the most accurate cardiovascular risk assessment possible."
    ...
    def _handle_general_question(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            # Create COMPLETE prompt with ENTIRE deidentified claims data (SINGLE LINE)
            complete_prompt = f"You are an expert medical claims data assistant with access to the COMPLETE, ENTIRE deidentified patient claims records (medical, pharmacy, and MCID). You have the FULL JSON data structures available. Answer the user's question with specific, detailed information from ANY part of the complete claims data provided. COMPLETE DEIDENTIFIED CLAIMS DATA (ENTIRE JSON STRUCTURES - MEDICAL & PHARMACY): {complete_context} RECENT CONVERSATION HISTORY: {history_text} USER QUESTION: {user_query} CRITICAL INSTRUCTIONS: - You have access to the COMPLETE deidentified medical, pharmacy, and MCID claims JSON data - Search through the ENTIRE JSON structure to find relevant information - Include specific dates (clm_rcvd_dt, rx_filled_dt), codes, medications, diagnoses, and values from ANY part of the JSON - Reference exact field names, values, and nested structures from the data - If user asks about ANY specific field, code, medication, or data point, find it in the complete JSON - Include ICD-10 codes, NDC codes, service codes, dates, quantities, and any other specific data - Access all nested levels of the JSON structure to answer questions - Be thorough and cite specific data points from the complete deidentified records - If data exists in the JSON, you can find and reference it - Use the conversation history to understand follow-up questions and context - Explain medical codes and terminology when relevant - For numerical values, dates, codes - provide exact values from the JSON data - Include both medical claims data AND pharmacy claims data in your analysis DETAILED ANSWER USING COMPLETE DEIDENTIFIED CLAIMS DATA:"
    ...
    def _create_comprehensive_trajectory_prompt(self, medical_data: Dict, pharmacy_data: Dict, mcid_data: Dict,
                                              ...
                                              entities: Dict) -> str:
        """Create comprehensive prompt for health trajectory analysis (SINGLE LINE)"""
        return f"You are a healthcare AI assistant analyzing a patient's comprehensive health trajectory. Based on the following complete deidentified claims data with comprehensive nested JSON processing, provide a detailed health trajectory analysis. COMPLETE DEIDENTIFIED MEDICAL CLAIMS DATA: {json.dumps(medical_data, indent=2, default=str)} COMPLETE DEIDENTIFIED PHARMACY CLAIMS DATA: {json.dumps(pharmacy_data, indent=2, default=str)} COMPLETE DEIDENTIFIED MCID CLAIMS DATA: {json.dumps(mcid_data, indent=2, default=str)} ENHANCED MEDICAL DATA EXTRACTIONS: {json.dumps(medical_extraction, indent=2)} ENHANCED PHARMACY DATA EXTRACTIONS: {json.dumps(pharmacy_extraction, indent=2)} COMPREHENSIVE HEALTH ENTITIES: {json.dumps(entities, indent=2)} Please analyze this patient's health trajectory focusing on: 1. **Current Health Status**: Comprehensive assessment based on medical codes, pharmacy data, and extracted entities 2. **Risk Factors**: Identified health risks from ICD-10 codes and medication patterns 3. **Medication Analysis**: Complete NDC codes, drug names, and therapeutic areas identified 4. **Chronic Conditions**: Long-term health management needs from medical service codes 5. **Health Trends**: Trajectory of health over time based on comprehensive service utilization 6. **Care Recommendations**: Suggested areas for medical attention based on complete data analysis 7. **Claims Data Integration**: How medical, pharmacy, and MCID data provide comprehensive view. Provide a detailed analysis (500-600 words) that synthesizes all the available structured and unstructured information into a coherent health trajectory assessment using the comprehensive claims data processing results."
    ...
    def _create_comprehensive_summary_prompt(self, trajectory_analysis: str, entities: Dict, 
        ...
        """Create comprehensive prompt for final health summary (SINGLE LINE)"""
        return f"Based on the detailed health trajectory analysis below and the comprehensive claims data extractions with enhanced processing, create a concise executive summary of this patient's health status. COMPREHENSIVE HEALTH TRAJECTORY ANALYSIS: {trajectory_analysis} ENHANCED HEALTH ENTITIES: - Diabetes: {entities.get('diabetics', 'unknown')} - Age Group: {entities.get('age_group', 'unknown')} - Smoking Status: {entities.get('smoking', 'unknown')} - Alcohol Status: {entities.get('alcohol', 'unknown')} - Blood Pressure: {entities.get('blood_pressure', 'unknown')} - Medical Conditions Identified: {len(entities.get('medical_conditions', []))} - Medications Identified: {len(entities.get('medications_identified', []))} ENHANCED MEDICAL CLAIMS DATA SUMMARY: - Health Service Records: {len(medical_extraction.get('hlth_srvc_records', []))} - Total Diagnosis Codes: {medical_extraction.get('extraction_summary', {}).get('total_diagnosis_codes', 0)} - Unique Service Codes: {len(medical_extraction.get('extraction_summary', {}).get('unique_service_codes', []))} ENHANCED PHARMACY CLAIMS DATA SUMMARY: - NDC Records: {len(pharmacy_extraction.get('ndc_records', []))} - Unique NDC Codes: {len(pharmacy_extraction.get('extraction_summary', {}).get('unique_ndc_codes', []))} - Unique Medications: {len(pharmacy_extraction.get('extraction_summary', {}).get('unique_label_names', []))} Create a comprehensive final summary that includes: 1. **Health Status Overview** (2-3 sentences based on comprehensive claims data) 2. **Key Risk Factors** (bullet points based on ICD-10 codes and enhanced medication analysis) 3. **Priority Recommendations** (3-4 actionable items based on comprehensive analysis) 4. **Follow-up Needs** (timing and type of care based on service codes and medication patterns) 5. **Claims Data Processing Notes** (brief note on comprehensive deidentification and extraction) Keep the summary under 300 words and focus on actionable insights for healthcare providers based on the comprehensive claims data analysis with enhanced nested JSON processing."
