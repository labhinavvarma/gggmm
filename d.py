...
with st.sidebar:
    ...
    if os.path.exists(heart_attack_combined_pkl_path):
        ...
        # PKL File Inspector
        if st.button("🔍 Inspect PKL File Structure", key="inspect_pkl"):
            try:
                # Create a temporary agent to use the inspector
                temp_config = Config(heart_attack_combined_pkl_path=heart_attack_combined_pkl_path)
                temp_agent = HealthAnalysisAgent(temp_config)
                
                inspection_result = temp_agent.inspect_pkl_file()
                
                st.markdown("**🔍 PKL File Inspection Results:**")
                
                if inspection_result.get("error"):
                    st.error(f"❌ Inspection Error: {inspection_result['error']}")
                else:
                    st.success(f"✅ File Size: {inspection_result['file_size']} bytes")
                    st.info(f"📋 Data Type: {inspection_result['data_type']}")
                    
                    structure = inspection_result.get("structure", {})
                    if structure:
                        st.markdown("**📊 Structure Analysis:**")
                        
                        if structure.get("type") == "dictionary":
                            st.write(f"**Type:** Dictionary with {len(structure.get('keys', []))} keys")
                            st.write(f"**Keys:** {structure.get('keys', [])}")
                            if structure.get('potential_model_keys'):
                                st.success(f"🤖 **Potential Model Keys:** {structure['potential_model_keys']}")
                            if structure.get('potential_scaler_keys'):
                                st.success(f"🔧 **Potential Scaler Keys:** {structure['potential_scaler_keys']}")
                            if structure.get('potential_feature_keys'):
                                st.success(f"📊 **Potential Feature Keys:** {structure['potential_feature_keys']}")
                                
                        elif structure.get("type") == "object_with_attributes":
                            st.write(f"**Type:** Object with {len(structure.get('attributes', []))} attributes")
                            st.write(f"**Attributes:** {structure.get('attributes', [])[:10]}...")
                            if structure.get('potential_model_attrs'):
                                st.success(f"🤖 **Potential Model Attributes:** {structure['potential_model_attrs']}")
                            if structure.get('potential_scaler_attrs'):
                                st.success(f"🔧 **Potential Scaler Attributes:** {structure['potential_scaler_attrs']}")
                                
                        elif structure.get("type") == "list_or_tuple":
                            st.write(f"**Type:** {structure.get('type').title()} with {structure.get('length', 0)} items")
                            st.write(f"**Item Types:** {structure.get('item_types', [])}")
                            
                        elif structure.get("type") == "direct_object":
                            st.write("**Type:** Direct Model Object")
                            st.write(f"**Has predict method:** {'✅' if structure.get('has_predict') else '❌'}")
                            st.write(f"**Has fit method:** {'✅' if structure.get('has_fit') else '❌'}")
                            st.write(f"**Has predict_proba method:** {'✅' if structure.get('has_predict_proba') else '❌'}")
                            if structure.get('module'):
                                st.write(f"**Module:** {structure['module']}")
                        ...
                        if structure.get("type") == "dictionary" and structure.get('potential_model_keys'):
                            st.info(f"✅ Try accessing model via: `data['{structure['potential_model_keys'][0]}']`")
                        elif structure.get("type") == "direct_object" and structure.get('has_predict'):
                            st.info("✅ This appears to be a direct model object - should work!")
                        else:
                            st.warning("⚠️ Unclear structure - manual inspection may be needed")
                            
            except Exception as e:
                st.error(f"❌ PKL inspection failed: {str(e)}")
        ...
        try:
            ...
            st.error("🔍 **Debug Info:**")
            st.error(f"- File exists: {os.path.exists(heart_attack_combined_pkl_path)}")
            st.error(f"- File size: {os.path.getsize(heart_attack_combined_pkl_path) if os.path.exists(heart_attack_combined_pkl_path) else 'N/A'} bytes")
            st.error(f"- Error type: {type(e).__name__}")
            st.error(f"- Error message: {str(e)}")
    else:
        st.warning(f"⚠️ Combined model file not found: {heart_attack_combined_pkl_path}")
        
        # Help section for PKL file issues
        with st.expander("🔧 PKL File Troubleshooting"):
            st.markdown("""
            **Common PKL File Issues:**
            
            1. **File Path**: Make sure the PKL file is in the correct location
            2. **File Format**: Ensure it's a valid pickle file (`.pkl` extension)
            3. **Python Version**: PKL files can be version-specific
            4. **Dependencies**: Make sure all required libraries are installed
            
            **Expected PKL Structure:**
            Your PKL file should contain one of:
            - Dictionary: `{'model': AdaBoostClassifier, 'scaler': StandardScaler, 'feature_names': [...]}`
            - Object with attributes: `obj.model`, `obj.scaler`, `obj.feature_names`
            - Tuple: `(model, scaler, features)`
            - Direct model: AdaBoostClassifier object
            ...
            **Quick Test:**
            ...
            with open('your_file.pkl', 'rb') as f:
                data = pickle.load(f)
            print(type(data))
            print(dir(data))  # if object
            print(data.keys())  # if dictionary
            ```
            """)
            
            # File re-upload option
            st.markdown("**🔄 Try Re-uploading Your PKL File:**")
            debug_upload = st.file_uploader(
                "Upload PKL for Debugging", 
                type=['pkl'], 
                key="debug_upload",
                help="Upload your PKL file to see detailed structure analysis"
            )
            
            if debug_upload:
                # Save temporarily and inspect
                debug_path = f"debug_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
                with open(debug_path, "wb") as f:
                    f.write(debug_upload.read())
                
                try:
                    temp_config = Config(heart_attack_combined_pkl_path=debug_path)
                    temp_agent = HealthAnalysisAgent(temp_config)
                    debug_result = temp_agent.inspect_pkl_file(debug_path)
                    ...
                    st.error(f"Debug inspection failed: {debug_error}")
