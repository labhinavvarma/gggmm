...
# Enhanced State Definition for LangGraph (8 nodes including FastAPI heart attack prediction)
class HealthAnalysisState(TypedDict):
    ...
    # NEW: FastAPI Heart Attack Prediction
...
class HealthAnalysisAgent:
    def __init__(self, custom_config: Optional[Config] = None):
        ...
        logger.info(f"🔗 FastAPI Server URL: {self.config.heart_attack_api_url}")
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(health_url) as response:
                    if response.status == 200:
                        ...
                        async with session.post(predict_url, json=test_data) as pred_response:
    ...
    async def _call_fastapi_heart_attack_prediction(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # FastAPI expects direct feature mapping
            payload = {
                "age": features.get("age", 50),
                "gender": features.get("gender", 0),
                "diabetes": features.get("diabetes", 0),
                "high_bp": features.get("high_bp", 0),
    ...
    def setup_langgraph(self):
        """Setup LangGraph workflow - 8 node enhanced workflow with FastAPI heart attack prediction"""
        ...
        workflow.add_node("predict_heart_attack_fastapi", self.predict_heart_attack_fastapi)  # NEW FASTAPI NODE
        ...
        workflow.add_conditional_edges(
            ...
            {
                "continue": "predict_heart_attack_fastapi",  # NEW: Go to FastAPI heart attack prediction
                "error": "handle_error"
            }
        )
        
        # NEW: FastAPI Heart attack prediction node routing
        workflow.add_conditional_edges(
            "predict_heart_attack_fastapi",
    ...
    # ===== LANGGRAPH NODES (keeping the same structure, just updating Node 8) =====
    ...
    def predict_heart_attack_fastapi(self, state: HealthAnalysisState) -> HealthAnalysisState:
        """LangGraph Node 8: Predict heart attack risk using FastAPI server and health data"""
        logger.info("❤️ LangGraph Node 8: Starting FastAPI heart attack prediction...")
        state["current_step"] = "predict_heart_attack_fastapi"
        state["step_status"]["predict_heart_attack_fastapi"] = "running"
        
        try:
            # Extract features from health data for FastAPI model (5 features)
            ...
            if not features or "error" in features:
                ...
                state["step_status"]["predict_heart_attack_fastapi"] = "error"
                return state
            
            # Prepare feature data for FastAPI server
            fastapi_features = self._prepare_fastapi_features(features)
            
            if fastapi_features is None:
                state["errors"].append("Failed to prepare features for FastAPI prediction")
                state["step_status"]["predict_heart_attack_fastapi"] = "error"
                return state
            
            # Make async call to FastAPI server (we need to handle this in sync context)
            try:
                # Create new event loop for the async call
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                fastapi_result = loop.run_until_complete(self._call_fastapi_heart_attack_prediction(fastapi_features))
                loop.close()
            except Exception as async_error:
                logger.error(f"Async FastAPI call failed: {async_error}")
                fastapi_result = {"success": False, "error": f"FastAPI call failed: {str(async_error)}"}
            
            # Process FastAPI prediction result
            prediction_result = self._process_fastapi_prediction_result(fastapi_result, state)
            
            state["heart_attack_prediction"] = prediction_result
            state["heart_attack_risk_score"] = prediction_result.get("risk_score", 0.0)
            
            state["step_status"]["predict_heart_attack_fastapi"] = "completed"
            logger.info("✅ FastAPI heart attack prediction completed successfully")
    ...
    def initialize_chatbot(self, state: HealthAnalysisState) -> HealthAnalysisState:
        """LangGraph Node 9: Initialize interactive chatbot with all context including FastAPI heart attack prediction"""
        ...
        try:
            # Prepare chatbot context with all data including FastAPI heart attack prediction
            chatbot_context = {
                "deidentified_medical": state.get("deidentified_medical", {}),
                "deidentified_pharmacy": state.get("deidentified_pharmacy", {}),
                "medical_extraction": state.get("medical_extraction", {}),
                ...
                # NEW: FastAPI heart attack prediction context
    ...
    def should_continue_after_heart_attack_prediction(self, state: HealthAnalysisState) -> Literal["continue", "error"]:
        """LangGraph Conditional: Decide what to do after FastAPI heart attack prediction"""
        return "error" if state["errors"] else "continue"
    
    # ===== FASTAPI HEART ATTACK PREDICTION METHODS =====
    
    def _extract_heart_attack_features_for_fastapi(self, state: HealthAnalysisState) -> Dict[str, Any]:
        """Extract features specifically for FastAPI model: Age, Gender, Diabetes, High_BP, Smoking"""
        try:
    ...
    def _process_fastapi_prediction_result(self, fastapi_result: Dict[str, Any], state: HealthAnalysisState) -> Dict[str, Any]:
        """Process FastAPI prediction result into comprehensive prediction object"""
        try:
            logger.info("🤖 Processing FastAPI heart attack prediction result...")
            
            if not fastapi_result.get("success", False):
                return {
                    "error": fastapi_result.get("error", "FastAPI call failed"),
                    "risk_score": 0.0,
                    "risk_level": "ERROR",
                    "risk_color": "gray",
                    "risk_icon": "❌",
                    "model_info": {
                        ...
                        "fastapi_server_url": self.config.heart_attack_api_url,
                        "error_details": fastapi_result.get("error", "Unknown error")
                    }
                }
            
            # Extract prediction data from FastAPI response
            prediction_data = fastapi_result.get("prediction_data", {})
            
            risk_score = prediction_data.get("risk_probability", 0.5)
            risk_level = prediction_data.get("risk_level", "MODERATE")
            confidence = prediction_data.get("confidence", risk_score)
            binary_prediction = prediction_data.get("binary_prediction", 0)
            message = prediction_data.get("message", "No message available")
            model_info_api = prediction_data.get("model_info", {})
            ...
            prediction_result = {
                ...
                "model_info": {
                    "model_type": model_info_api.get("model_type", "AdaBoost_FastAPI_Server"),
                    "model_source": "fastapi_server",
                    "fastapi_server_url": self.config.heart_attack_api_url,
                    "features_used": 5,
                    "feature_names": ["Age", "Gender", "Diabetes", "High_BP", "Smoking"],
                    "input_features": model_info_api.get("input_features", {}),
                    "threshold": self.config.heart_attack_threshold,
                    "prediction_timestamp": datetime.now().isoformat()
                }
            }
            
            logger.info(f"✅ FastAPI heart attack prediction processed")
            logger.info(f"❤️ Risk Score: {risk_score:.3f} ({risk_level})")
            
            return prediction_result
            
        except Exception as e:
            logger.error(f"Error processing FastAPI prediction result: {e}")
            return {
                "error": f"FastAPI result processing failed: {str(e)}",
                "risk_score": 0.0,
                "risk_level": "ERROR",
                "risk_color": "gray",
                "risk_icon": "❌",
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        ...
        try:
            # Prepare context with medical, pharmacy, and FastAPI heart attack prediction data
            json_context = {
...
Here are the complete deidentified medical records, analysis, and FastAPI server heart attack risk prediction for this patient:
...
3. FastAPI server heart attack risk assessment and contributing factors
...
Always maintain patient privacy and provide evidence-based responses using the medical data and FastAPI predictions provided."""
    ...
    # ===== HELPER METHODS (keeping existing methods for brevity) =====
...
"""
    ...
    def run_analysis(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {
                ...
                # NEW: FastAPI heart attack prediction results
    ...
    def _count_completed_steps(self, state: HealthAnalysisState) -> int:
        ...
        if state.get("heart_attack_prediction"): steps += 1  # NEW: Count FastAPI heart attack prediction
...
def main():
    ...
    print("❤️ Heart Attack Prediction using FastAPI server (AdaBoost model)")
    ...
    config = Config(
        heart_attack_api_url="http://localhost:8000"  # User will specify this
    )
    ...
    print(f"   🔗 FastAPI Server URL: {config.heart_attack_api_url}")
