...
def display_api_responses_collapsed(api_responses: Dict[str, Any]):
    """Display API responses in collapsed format at the top - ALWAYS VISIBLE"""
    if not api_responses:
        st.info("ℹ️ No API responses available yet")
        return
    
    st.markdown("### 📡 **Raw API Responses from MCP Server**")
    st.markdown("*Click on the tabs below to view the raw JSON responses from each healthcare API*")
    
    # Create tabs for different API responses
    available_apis = []
    api_data = {}
    
    # Check which APIs have data
    for api_name in ["mcid", "medical", "token", "pharmacy", "all"]:
        if api_name in api_responses and api_responses[api_name]:
            available_apis.append(api_name)
            api_data[api_name] = api_responses[api_name]
    
    if not available_apis:
        st.warning("⚠️ No API response data found")
        st.json(api_responses)  # Show whatever we have
        return
    
    # Create tabs for available APIs
    tab_labels = []
    for api in available_apis:
        if api == "mcid":
            tab_labels.append("🆔 MCID")
        elif api == "medical": 
            tab_labels.append("🏥 Medical")
        elif api == "token":
            tab_labels.append("🔑 Token")
        elif api == "pharmacy":
            tab_labels.append("💊 Pharmacy")
        elif api == "all":
            tab_labels.append("📊 All Data")
        else:
            tab_labels.append(f"🔧 {api.title()}")
    
    api_tabs = st.tabs(tab_labels)
    
    for i, (api_name, tab) in enumerate(zip(available_apis, api_tabs)):
        with tab:
            api_response = api_data[api_name]
            
            # Show status and basic info
            status_code = api_response.get('status_code', 'Unknown')
            endpoint = api_response.get('endpoint', 'Unknown')
            ...
            # Show payload sent (if available)
            if 'payload_sent' in api_response:
                with st.expander("📤 Payload Sent to MCP Server", expanded=True):
                    st.json(api_response['payload_sent'])
            
            # Show response body
            if 'body' in api_response:
                with st.expander("📥 Raw JSON Response from MCP Server", expanded=True):
                    st.json(api_response['body'])
            elif 'error' in api_response:
                with st.expander("❌ Error Response", expanded=True):
                    st.error(api_response['error'])
                    if 'suggestion' in api_response:
                        st.info(f"💡 Suggestion: {api_response['suggestion']}")
            
            # Show full response object for debugging
            with st.expander("🔍 Complete API Response Object", expanded=False):
                st.json(api_response)
    
    # Summary metrics
    st.markdown("#### 📊 **API Call Summary**")
    successful_apis = sum(1 for api_data in api_responses.values() 
                         if api_data and api_data.get('status_code') == 200)
    total_apis = len([api for api in api_responses.values() if api])
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("APIs Called", total_apis)
    with col2:
        st.metric("Successful", successful_apis)
    with col3:
        failed_apis = total_apis - successful_apis
        st.metric("Failed", failed_apis)
    with col4:
    ...
    for api_name, response in api_responses.items():
        if response and response.get('status_code') != 200:
            error_apis.append(f"{api_name}: {response.get('error', 'Unknown error')}")
    
    if error_apis:
        st.markdown("#### ❌ **API Errors**")
        for error in error_apis:
            st.error(f"• {error}")
    
    st.markdown("---")

def display_analysis_results(result: Dict[str, Any]):
    """Display comprehensive analysis results with fixed error handling"""
    ...
    if health_analysis:
        ...
        with col3:
            # Fix the TypeError by handling different data types
            risk_factors = summary.get('risk_factors', {})
            if isinstance(risk_factors, dict):
                risk_count = len(risk_factors)
            elif isinstance(risk_factors, (list, tuple)):
                risk_count = len(risk_factors)
            elif isinstance(risk_factors, int):
                risk_count = risk_factors
            else:
                risk_count = 0
            st.metric("Risk Factors", risk_count)
        with col4:
...
def display_entity_extraction(entity_extraction: Dict[str, Any]):
    """Display entity extraction results with error handling"""
            ...
            for i, (condition, status) in enumerate(conditions.items()):
                with condition_cols[i % 4]:
                    ...
                    <div class="entity-card">
                        <strong>{str(condition).title()}</strong><br>
                        <span style="color: #dc3545;">{str(status)}</span>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        if medications and isinstance(medications, dict):
            ...
            for i, (med, status) in enumerate(medications.items()):
                with med_cols[i % 4]:
                    st.markdown(f"""
                    <div class="entity-card">
                        <strong>{str(med).title()}</strong><br>
                        <span style="color: #28a745;">{str(status)}</span>
                    </div>
                    """, unsafe_allow_html=True)
        
        # Show raw entity data in expander for debugging
        with st.expander("🔍 Raw Entity Extraction Data"):
            st.json(entity_extraction)
            
    except Exception as e:
        st.error(f"⚠️ Error displaying entity extraction: {str(e)}")
        with st.expander("🐛 Debug: Entity Extraction Data"):
            st.json(entity_extraction)

def display_workflow_steps(step_status: Dict[str, str]):
    """Display workflow step progress with error handling"""
    if not step_status:
        return
    ...
    try:
        ...
        st.error(f"⚠️ Error displaying workflow steps: {str(e)}")
        with st.expander("🐛 Debug: Step Status Data"):
            st.json(step_status)

def display_deidentified_data(deidentified_data: Dict[str, Any]):
    """Display deidentified data summary with error handling"""
        ...
        with st.expander("🔒 **Deidentified Data Summary**"):
            for data_type, data in deidentified_data.items():
                ...
                if isinstance(data, dict) and not data.get('error'):
                    ...
                    error_msg = data.get('error', 'Unknown error') if isinstance(data, dict) else 'Processing error'
                    st.markdown(f"⚠️ {error_msg}")
                    
    except Exception as e:
        st.error(f"⚠️ Error displaying deidentified data: {str(e)}")
        with st.expander("🐛 Debug: Deidentified Data"):
...
• **Raw JSON Response Display** - See actual API responses from MCP server
...
💡 **You'll see raw JSON responses from MCID, Medical, Token, and Pharmacy APIs after each analysis!**
...
for i, message in enumerate(st.session_state.messages):
    ...
    # Display raw API responses for assistant messages with analysis results
    if message["role"] == "assistant" and i < len(st.session_state.analysis_results):
        result = st.session_state.analysis_results[i]
        
        # ALWAYS show API responses first if available
        if result and result.get('api_responses'):
            # Display raw API responses in prominent collapsed format
            st.markdown("---")  # Visual separator
            display_api_responses_collapsed(result.get('api_responses', {}))
            st.markdown("---")  # Visual separator
...
if prompt := st.chat_input("Ask about healthcare analysis, patient data, or medical information..."):
    ...
    with st.spinner("🤖 Processing your request through Enhanced LangGraph workflow..."):
        try:
            ...
            # ALWAYS display raw API responses FIRST and PROMINENTLY if available
            if result.get('api_responses'):
                st.markdown("---")  # Visual separator
                st.markdown("🔍 **MCP Server Response Data Below:**")
                display_api_responses_collapsed(result.get('api_responses', {}))
                st.markdown("---")  # Visual separator
            ...
            if result.get('analysis_complete'):
                try:
                    ...
                    st.error(f"⚠️ Error displaying analysis results: {str(e)}")
                    # Show debug info
                    with st.expander("🐛 Debug: Analysis Result Structure"):
                        st.json(result.get('health_analysis', {}))
            ...
            with st.expander("🐛 Detailed Error Information"):
                ...
                st.markdown("1. Check if the MCP server is running on port 8000")
                st.markdown("2. Verify the server logs for any errors")
                st.markdown("3. Try running the health check in the sidebar")
                st.markdown("4. Restart the server if needed")
            ...
            st.session_state.analysis_results.append({"errors": [str(e)]})
