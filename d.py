...
class HealthAnalysisAgent:
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            # Create focused prompt for specific questions - FIXED: More concise responses
            full_prompt = f"""{self.config.chatbot_sys_msg}

You have access to comprehensive deidentified medical records and heart attack risk prediction data for this patient. Answer the user's specific question directly and concisely based on the relevant data.

AVAILABLE DATA:
- Medical records with ICD-10 diagnosis codes and health service codes
- Pharmacy records with NDC codes and medication names  
- Health entity extraction results
- Heart attack risk prediction from FastAPI model
- Health trajectory analysis and clinical summary

MEDICAL DATA: {json.dumps(json_context.get("deidentified_medical_data", {}), indent=1)}
PHARMACY DATA: {json.dumps(json_context.get("deidentified_pharmacy_data", {}), indent=1)}
HEALTH ENTITIES: {json.dumps(json_context.get("health_entities", {}), indent=1)}
HEART ATTACK PREDICTION: {json.dumps(json_context.get("heart_attack_prediction", {}), indent=1)}
...
INSTRUCTIONS:
- Answer the specific question asked - do not provide a full medical summary unless requested
- Use only relevant data from the records above
- Be concise and focused on the user's question
- If the question is about medications, focus on pharmacy data
- If about diagnoses, focus on medical codes
- If about heart attack risk, focus on the prediction data
- Maintain patient privacy and provide professional insights"""
    ...
    def _recursive_medical_extraction(self, data: Any, result: Dict[str, Any], path: str = ""):
        ...
        if isinstance(data, dict):
            ...
            # FIXED: Handle both individual diag_1_cd to diag_50_cd and comma-separated diag_1_50_cd
            for i in range(1, 51):
            ...
            # FIXED: Handle comma-separated diagnosis codes in diag_1_50_cd format
            if "diag_1_50_cd" in data and data["diag_1_50_cd"]:
                comma_separated_codes = str(data["diag_1_50_cd"]).strip()
                if comma_separated_codes:
                    # Split by comma and process each code
                    individual_codes = [code.strip() for code in comma_separated_codes.split(",") if code.strip()]
                    for idx, code in enumerate(individual_codes, 1):
                        diagnosis_codes.append({
                            "code": code,
                            "position": idx,
                            "source": "diag_1_50_cd_split"
                        })
                        result["extraction_summary"]["unique_diagnosis_codes"].add(str(code))
