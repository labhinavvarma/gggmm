...
@dataclass
class Config:
    fastapi_url: str = "http://localhost:8080"
...
class HealthAnalysisAgent:
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Handle chatbot conversation with focused, direct responses"""
        try:
            # Prepare focused context based on user query
            query_lower = user_query.lower()
            
            # Determine what data is most relevant to the query
            relevant_context = {}
            
            if any(word in query_lower for word in ['medication', 'drug', 'prescription', 'pharmacy', 'ndc']):
                relevant_context["pharmacy_data"] = chat_context.get("pharmacy_extraction", {})
                relevant_context["medications"] = chat_context.get("entity_extraction", {}).get("medications_identified", [])
                
            if any(word in query_lower for word in ['diagnosis', 'condition', 'medical', 'icd', 'disease']):
                relevant_context["medical_data"] = chat_context.get("medical_extraction", {})
                relevant_context["conditions"] = chat_context.get("entity_extraction", {}).get("medical_conditions", [])
                
            if any(word in query_lower for word in ['heart attack', 'cardiac', 'risk', 'prediction', 'heart']):
                relevant_context["heart_attack_prediction"] = chat_context.get("heart_attack_prediction", {})
                relevant_context["heart_attack_features"] = chat_context.get("heart_attack_features", {})
                
            if any(word in query_lower for word in ['summary', 'overview', 'trajectory', 'analysis']):
                relevant_context["health_trajectory"] = chat_context.get("health_trajectory", "")
                relevant_context["final_summary"] = chat_context.get("final_summary", "")
                
            if any(word in query_lower for word in ['diabetes', 'smoking', 'blood pressure', 'age', 'entities']):
                relevant_context["health_entities"] = chat_context.get("entity_extraction", {})
            
            # If no specific focus, provide patient overview
            if not relevant_context:
                relevant_context["patient_overview"] = chat_context.get("patient_overview", {})
                relevant_context["key_entities"] = chat_context.get("entity_extraction", {})
            ...
            # Create focused prompt - much simpler
            focused_prompt = f"""You are a medical data assistant. Answer the user's question directly and concisely using only the relevant medical data provided.
...
- Be direct and concise (1-3 sentences unless more detail is specifically requested)
- Use the conversation history for context if the user refers to previous topics
- For heart attack risk questions, use the FastAPI server prediction results
- Do not provide unsolicited medical advice or interpretations
- Do not mention FastAPI technical details unless asked
- Focus solely on answering the specific question

ANSWER:"""

            logger.info(f"💬 Processing focused query: {user_query[:50]}...")
            
            # Call Snowflake Cortex with focused prompt
            response = self.call_llm(focused_prompt)
            
            if response.startswith("Error"):
                return f"Sorry, I encountered an error processing your question."
