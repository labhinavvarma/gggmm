...
"""
    ...
    def generate_response(self, state: ChatbotHealthState) -> ChatbotHealthState:
        ...
        try:
            if state.get("analysis_ready"):
                ...
                response = "Hello! I'm your healthcare analysis assistant. You can ask me to analyze patient data by providing details like: 'Analyze patient John Smith, DOB 1980-01-15, male, SSN 123456789, zip code 12345'"
    ...
    def route_user_input(self, state: ChatbotHealthState) -> Literal["extract_data", "contextual_chat", "general_response"]:
        ...
        # Check if we have existing analysis context for contextual chat
        if self.current_analysis_context:
            question_keywords = [
                "what", "how", "why", "explain", "tell me", "show me", 
                "medication", "condition", "risk", "diabetes", "blood pressure"
            ]
            
            if any(keyword in user_message for keyword in question_keywords):
                logger.info("🔄 Routing to contextual chat")
...
{json.dumps(self.current_analysis_context['deidentified_medical'], indent=2)[:1000]}...

DEIDENTIFIED PHARMACY DATA:
{json.dumps(self.current_analysis_context['deidentified_pharmacy'], indent=2)[:1000]}...

ENTITY EXTRACTION RESULTS:
{json.dumps(self.current_analysis_context['entity_extraction'], indent=2)}
"""
            
            prompt = f"""You are a healthcare AI assistant discussing analysis results with a user. Answer their question based on the deidentified patient data provided.
...
Instructions:
1. Answer based on the deidentified medical and pharmacy data
2. Reference specific findings when relevant  
3. Maintain conversation context from previous messages
4. Be concise but informative
5. Provide actionable insights when appropriate
6. Always mention this is based on deidentified data
7. If asked about raw data, explain you have access to the JSON responses
