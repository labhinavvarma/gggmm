...
# Custom CSS for clean layout and stable animation
st.markdown("""
<style>
.main-header {
    font-size: 2.5rem;
...
/* Stable Deep Research Animation */
...
def initialize_session_state():
    ...
    # Stable Animation Workflow Steps
    if 'workflow_steps' not in st.session_state:
        st.session_state.workflow_steps = {
            1: {'name': 'Fetching Claims Data', 'status': 'pending'},
            2: {'name': 'Deidentifying Claims Data', 'status': 'pending'},
            3: {'name': 'Extracting Claims Fields', 'status': 'pending'},
            4: {'name': 'Extracting Health Entities', 'status': 'pending'},
            5: {'name': 'Analyzing Health Trajectory', 'status': 'pending'},
            6: {'name': 'Generating Summary', 'status': 'pending'},
            7: {'name': 'Predicting Heart Attack Risk', 'status': 'pending'},
            8: {'name': 'Initializing Assistant', 'status': 'pending'}
        }
...
def update_workflow_step(step_number: int, status: str):
    """Safely update workflow step status"""
    try:
...
def display_stable_workflow_animation():
    """Display stable workflow animation with current progress"""
    try:
        ...
        # Create the complete animation HTML
        animation_html = f"""
        <div class="deep-research-container">
            <h3 style="margin-bottom: 1.5rem; color: white; text-align: center;">
            ...
            <div style="background: rgba(255, 255, 255, 0.1); padding: 1.5rem; border-radius: 8px; margin: 1rem 0;">
                <div style="text-align: center; margin-bottom: 1.5rem; font-size: 1.1rem; font-weight: 600;">
                    Processing Step {st.session_state.current_step} of 8
                </div>
                {steps_html}
                <div style="text-align: center; margin-top: 1.5rem; font-style: italic; opacity: 0.9;">
        ...
        logger.warning(f"Error creating animation HTML: {e}")
        return """
        <div class="deep-research-container">
...
# Analysis Status Display with Stable Animation
...
if st.session_state.analysis_running and st.session_state.show_animation:
    ...
    with animation_container.container():
        st.markdown(display_stable_workflow_animation(), unsafe_allow_html=True)
...
if submitted and not st.session_state.analysis_running:
    ...
    if not is_valid:
        ...
        st.info("🔬 Starting Deep Research Analysis - Please wait while we process your data...")
        st.warning("⏳ This may take 30-60 seconds. Watch the progress below:")
        
        # Start the workflow animation
        update_workflow_step(1, 'running')
        time.sleep(0.5)  # Brief pause to show initial step
        
        with st.spinner("🔬 Deep Research Analysis executing..."):
            try:
                # Execute the actual analysis in background
                results = st.session_state.agent.run_analysis(patient_data)
                
                # Simulate progress animation while analysis completes
                step_delays = [0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3]
                
                for step_num in range(1, 8):
                    update_workflow_step(step_num, 'completed')
                    update_workflow_step(step_num + 1, 'running')
                    time.sleep(step_delays[step_num - 1])
                
                # Complete final step
                update_workflow_step(8, 'completed')
                time.sleep(0.5)
