...
DETAILED ANSWER USING COMPLETE DEIDENTIFIED CLAIMS DATA:"""
    ...
    def get_code_explanations_for_record(self, record: Dict[str, Any], record_type: str = "medical") -> Dict[str, Any]:
        """Get LLM explanations for codes in a record"""
        explanations = {}
        
        try:
            if record_type == "medical":
                # Explain service code
                service_code = record.get("hlth_srvc_cd")
                if service_code:
                    explanations["service_code_explanation"] = self.data_processor.get_service_code_explanation(service_code)
                
                # Explain diagnosis codes
                diagnosis_codes = record.get("diagnosis_codes", [])
                explanations["diagnosis_explanations"] = []
                for diag in diagnosis_codes:
                    diag_code = diag.get("code")
                    if diag_code:
                        explanation = self.data_processor.get_diagnosis_code_explanation(diag_code)
                ...
                if ndc_code:
                    explanations["ndc_explanation"] = self.data_processor.get_ndc_code_explanation(ndc_code)
                
                # Explain medication
                medication = record.get("lbl_nm")
                if medication:
                    explanations["medication_explanation"] = self.data_processor.get_medication_explanation(medication)
