...
class HealthAnalysisAgent:
    ...
    def inspect_pkl_file(self, pkl_path: str = None) -> Dict[str, Any]:
        """Inspect PKL file structure for debugging purposes"""
        pkl_path = pkl_path or self.config.heart_attack_combined_pkl_path
        ...
        try:
            if not os.path.exists(pkl_path):
                inspection_result["error"] = f"File not found: {pkl_path}"
                return inspection_result
            
            inspection_result["file_exists"] = True
            inspection_result["file_size"] = os.path.getsize(pkl_path)
            
            # Try to load the PKL file
            with open(pkl_path, 'rb') as f:
                data = pickle.load(f)
            
            inspection_result["data_type"] = str(type(data))
            
            # Inspect based on data type
            if isinstance(data, dict):
                inspection_result["structure"] = {
                    "type": "dictionary",
                    "keys": list(data.keys()),
                    "key_types": {k: str(type(v)) for k, v in data.items()},
                    "potential_model_keys": [k for k in data.keys() if 'model' in k.lower() or 'class' in k.lower() or 'estim' in k.lower()],
                    "potential_scaler_keys": [k for k in data.keys() if 'scal' in k.lower() or 'preproc' in k.lower() or 'normal' in k.lower()],
                    "potential_feature_keys": [k for k in data.keys() if 'feature' in k.lower() or 'column' in k.lower()]
                }
                ...
                inspection_result["structure"] = {
                    ...
                    "attributes": attrs,
                    "attr_types": {attr: str(type(getattr(data, attr, None))) for attr in attrs[:10]},  # Limit to first 10
                    "potential_model_attrs": [attr for attr in attrs if 'model' in attr.lower() or 'class' in attr.lower()],
                    "potential_scaler_attrs": [attr for attr in attrs if 'scal' in attr.lower() or 'preproc' in attr.lower()],
                    "potential_feature_attrs": [attr for attr in attrs if 'feature' in attr.lower() or 'column' in attr.lower()]
                }
                ...
                inspection_result["structure"] = {
                    "type": "list_or_tuple",
                    "length": len(data),
                    "item_types": [str(type(item)) for item in data[:5]],  # First 5 items
                    "has_predict_method": [hasattr(item, 'predict') if hasattr(item, '__dict__') else False for item in data[:3]]
                }
                ...
                inspection_result["structure"] = {
                    ...
                    "object_methods": [method for method in dir(data) if not method.startswith('_') and callable(getattr(data, method, None))][:10]
                }
                
                # Check if it's a scikit-learn model
                if hasattr(data, '__module__'):
                    inspection_result["structure"]["module"] = data.__module__
                
        except Exception as e:
            inspection_result["error"] = f"Error inspecting PKL file: {str(e)}"
