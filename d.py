...
# Enhanced Configuration with MCP Heart Attack Prediction
@dataclass
class Config:
...
# Enhanced State Definition for LangGraph (8 nodes including MCP heart attack prediction)
class HealthAnalysisState(TypedDict):
    ...
    # NEW: MCP Heart Attack Prediction
...
class HealthAnalysisAgent:
    def __init__(self, custom_config: Optional[Config] = None):
        ...
        logger.info("🔧 HealthAnalysisAgent initialized with Snowflake Cortex API + Interactive Chatbot + MCP Heart Attack Prediction")
        logger.info(f"🌐 API URL: {self.config.api_url}")
        logger.info(f"🤖 Model: {self.config.model}")
        logger.info(f"🔑 App ID: {self.config.app_id}")
        logger.info(f"💬 Chatbot: Enhanced Interactive Mode")
        logger.info(f"❤️ Heart Attack Prediction: MCP Server Enabled")
    ...
    async def test_mcp_connection(self) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"🧪 Testing MCP server connection at {self.config.mcp_server_url}...")
            ...
            result = await self._call_mcp_heart_attack_prediction(test_data)
            ...
            if "error" in result:
                ...
                return {
                    "success": True,
                    "response": result.get("prediction", "Test successful"),
                    "mcp_server_url": self.config.mcp_server_url,
                    "test_data": test_data
                }
    ...
    async def _call_mcp_heart_attack_prediction(self, features: Dict[str, Any]) -> Dict[str, Any]:
        """Call MCP server for heart attack prediction"""
        try:
            logger.info(f"🔗 Calling MCP server for heart attack prediction...")
            logger.info(f"📊 Features: {features}")
            
            # Prepare the request to MCP server
            mcp_url = f"{self.config.mcp_server_url}/tools/call"
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(mcp_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        # Extract prediction from MCP response
                        if "content" in result and result["content"]:
                            prediction_text = result["content"][0].get("text", "")
                            return self._parse_mcp_prediction_response(prediction_text)
                        else:
                            return {"error": "No content in MCP response"}
                    else:
                        error_text = await response.text()
                        return {"error": f"MCP server error {response.status}: {error_text}"}
    ...
    def _parse_mcp_prediction_response(self, prediction_text: str) -> Dict[str, Any]:
        """Parse the MCP prediction response text"""
        try:
            # Example response: "Heart Disease Risk: 65.2% (High Risk) | Confidence: 65.2%"
            logger.info(f"📋 Parsing MCP response: {prediction_text}")
            ...
            if "High Risk" in prediction_text:
                ...
                risk_level = "LOW"
            elif "Medium Risk" in prediction_text:
                risk_level = "MODERATE"
            
            # Extract confidence
            conf_match = re.search(r'Confidence: (\d+\.?\d*)%', prediction_text)
            confidence = float(conf_match.group(1)) / 100 if conf_match else probability
    ...
    def setup_langgraph(self):
        """Setup LangGraph workflow - 8 node enhanced workflow with MCP heart attack prediction"""
        logger.info("🔧 Setting up Enhanced LangGraph workflow with 8 nodes (including MCP heart attack prediction)...")
        ...
        workflow.add_node("predict_heart_attack_mcp", self.predict_heart_attack_mcp)  # NEW MCP NODE
        ...
        workflow.add_conditional_edges(
            ...
            {
                "continue": "predict_heart_attack_mcp",  # NEW: Go to MCP heart attack prediction
                "error": "handle_error"
            }
        )
        
        # NEW: MCP Heart attack prediction node routing
        workflow.add_conditional_edges(
            "predict_heart_attack_mcp",
        ...
        logger.info("✅ Enhanced LangGraph workflow compiled successfully with 8 nodes including MCP heart attack prediction!")
    ...
    # ===== LANGGRAPH NODES (8 NODES INCLUDING MCP HEART ATTACK PREDICTION) =====
    ...
    def predict_heart_attack_mcp(self, state: HealthAnalysisState) -> HealthAnalysisState:
        """LangGraph Node 8: Predict heart attack risk using MCP server and health data"""
        logger.info("❤️ LangGraph Node 8: Starting MCP heart attack prediction...")
        state["current_step"] = "predict_heart_attack_mcp"
        state["step_status"]["predict_heart_attack_mcp"] = "running"
        
        try:
            # Extract features from health data for MCP model (5 features)
            features = self._extract_heart_attack_features_for_mcp(state)
            state["heart_attack_features"] = features
            
            if not features or "error" in features:
                state["errors"].append("Failed to extract features for MCP heart attack prediction")
                state["step_status"]["predict_heart_attack_mcp"] = "error"
                return state
            
            # Prepare feature data for MCP server
            mcp_features = self._prepare_mcp_features(features)
            
            if mcp_features is None:
                state["errors"].append("Failed to prepare features for MCP prediction")
                state["step_status"]["predict_heart_attack_mcp"] = "error"
                return state
            
            # Make async call to MCP server (we need to handle this in sync context)
            try:
                # Create new event loop for the async call
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                mcp_result = loop.run_until_complete(self._call_mcp_heart_attack_prediction(mcp_features))
                loop.close()
            except Exception as async_error:
                logger.error(f"Async MCP call failed: {async_error}")
                mcp_result = {"error": f"MCP call failed: {str(async_error)}"}
            
            # Process MCP prediction result
            prediction_result = self._process_mcp_prediction_result(mcp_result, state)
            
            state["heart_attack_prediction"] = prediction_result
            state["heart_attack_risk_score"] = prediction_result.get("risk_score", 0.0)
            
            state["step_status"]["predict_heart_attack_mcp"] = "completed"
            logger.info("✅ MCP heart attack prediction completed successfully")
            ...
            error_msg = f"Error in MCP heart attack prediction: {str(e)}"
            state["errors"].append(error_msg)
            state["step_status"]["predict_heart_attack_mcp"] = "error"
            logger.error(error_msg)
    ...
    def initialize_chatbot(self, state: HealthAnalysisState) -> HealthAnalysisState:
        """LangGraph Node 9: Initialize interactive chatbot with all context including MCP heart attack prediction"""
        logger.info("💬 LangGraph Node 9: Initializing interactive chatbot with MCP heart attack prediction...")
        state["current_step"] = "initialize_chatbot"
        state["step_status"]["initialize_chatbot"] = "running"
        
        try:
            # Prepare chatbot context with all data including MCP heart attack prediction
            chatbot_context = {
                "deidentified_medical": state.get("deidentified_medical", {}),
                "deidentified_pharmacy": state.get("deidentified_pharmacy", {}),
                "medical_extraction": state.get("medical_extraction", {}),
                ...
                # NEW: MCP heart attack prediction context
            ...
            logger.info("✅ Successfully initialized interactive chatbot with MCP heart attack prediction context")
            logger.info(f"💬 Chatbot ready with {len(chatbot_context)} data components including MCP heart attack prediction")
    ...
    def should_continue_after_heart_attack_prediction(self, state: HealthAnalysisState) -> Literal["continue", "error"]:
        """LangGraph Conditional: Decide what to do after MCP heart attack prediction"""
        return "error" if state["errors"] else "continue"
    
    # ===== MCP HEART ATTACK PREDICTION METHODS =====
    
    def _extract_heart_attack_features_for_mcp(self, state: HealthAnalysisState) -> Dict[str, Any]:
        """Extract features specifically for MCP model: Age, Gender, Diabetes, High_BP, Smoking"""
        try:
            logger.info("🔍 Extracting features for MCP heart attack prediction model...")
            ...
            logger.info(f"✅ Extracted {len(features)} features for MCP heart attack prediction")
            ...
            logger.error(f"Error extracting heart attack features for MCP model: {e}")
            return {"error": f"Feature extraction failed: {str(e)}"}
    
    def _prepare_mcp_features(self, features: Dict[str, Any]) -> Optional[Dict[str, int]]:
        """Prepare feature data for MCP server call"""
        try:
            extracted_features = features.get("extracted_features", {})
            
            # Prepare features for MCP server: age, gender, diabetes, high_bp, smoking
            mcp_features = {
                "age": extracted_features.get("Age", 50),
                "gender": extracted_features.get("Gender", 0),
                "diabetes": extracted_features.get("Diabetes", 0),
                "high_bp": extracted_features.get("High_BP", 0),
                "smoking": extracted_features.get("Smoking", 0)
            }
            
            logger.info(f"✅ MCP features prepared: {mcp_features}")
            return mcp_features
            
        except Exception as e:
            logger.error(f"Error preparing MCP features: {e}")
            return None
    
    def _process_mcp_prediction_result(self, mcp_result: Dict[str, Any], state: HealthAnalysisState) -> Dict[str, Any]:
        """Process MCP prediction result into comprehensive prediction object"""
        try:
            logger.info("🤖 Processing MCP heart attack prediction result...")
            ...
            if "error" in mcp_result:
                return {
                    ...
                    "risk_icon": "❌",
                    "model_info": {
                        "model_source": "mcp_server",
                        "mcp_server_url": self.config.mcp_server_url,
                        "error_details": mcp_result["error"]
                    }
                }
            
            # Extract prediction data
            risk_score = mcp_result.get("probability", 0.5)
            risk_level = mcp_result.get("risk_level", "MODERATE")
            confidence = mcp_result.get("confidence", risk_score)
            prediction_text = mcp_result.get("prediction", "No prediction available")
            ...
            prediction_result = {
                ...
                "prediction_interpretation": {
                    "risk_assessment": f"The MCP ML model predicts a {risk_level} risk of heart attack",
                    "confidence": f"{confidence * 100:.1f}% confidence",
                    "recommendation": self._get_risk_recommendation(risk_level),
                    "risk_factors": self._identify_risk_factors_for_mcp(state)
                },
                "model_info": {
                    "model_type": "AdaBoost_MCP_Server",
                    "model_source": "mcp_server",
                    "mcp_server_url": self.config.mcp_server_url,
                    "features_used": 5,
                    "feature_names": ["Age", "Gender", "Diabetes", "High_BP", "Smoking"],
                    "threshold": self.config.heart_attack_threshold,
                    "prediction_timestamp": datetime.now().isoformat()
                }
            }
            
            logger.info(f"✅ MCP heart attack prediction processed")
            logger.info(f"❤️ Risk Score: {risk_score:.3f} ({risk_level})")
            
            return prediction_result
            
        except Exception as e:
            logger.error(f"Error processing MCP prediction result: {e}")
            return {
                "error": f"MCP result processing failed: {str(e)}",
                "risk_score": 0.0,
                "risk_level": "ERROR",
                "risk_color": "gray",
                "risk_icon": "❌",
    ...
    def _identify_risk_factors_for_mcp(self, state: HealthAnalysisState) -> List[str]:
        """Identify key risk factors from the MCP model analysis"""
        risk_factors = []
    ...
    def chat_with_data(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Handle chatbot conversation with deidentified medical data context including MCP heart attack prediction"""
        try:
            # Prepare context with medical, pharmacy, and MCP heart attack prediction data
            json_context = {
            ...
            # Create comprehensive prompt with medical and MCP heart attack prediction context
            full_prompt = f"""{self.config.chatbot_sys_msg}

Here are the complete deidentified medical records, analysis, and MCP server heart attack risk prediction for this patient:
...
3. MCP server heart attack risk assessment and contributing factors
4. The specific features used in the MCP model: Age, Gender, Diabetes, High_BP, Smoking
5. Potential health implications
6. Professional medical insights based on the available data and MCP predictions

Always maintain patient privacy and provide evidence-based responses using the medical data and MCP predictions provided."""

            logger.info(f"💬 Processing chatbot query with MCP heart attack prediction context: {user_query[:100]}...")
    ...
    # ===== HELPER METHODS (keeping existing methods) =====
...
"""
    ...
    def run_analysis(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """Run the enhanced health analysis workflow using LangGraph with MCP heart attack prediction"""
        ...
        try:
            ...
            logger.info("🚀 Starting Enhanced LangGraph health analysis workflow with MCP heart attack prediction...")
            ...
            results = {
                ...
                # NEW: MCP heart attack prediction results
            ...
            if results["success"]:
                logger.info("✅ Enhanced LangGraph health analysis with MCP heart attack prediction completed successfully!")
    ...
    def _count_completed_steps(self, state: HealthAnalysisState) -> int:
        ...
        if state.get("heart_attack_prediction"): steps += 1  # NEW: Count MCP heart attack prediction
        if state.get("chatbot_ready"): steps += 1
        return steps

def main():
    """Example usage of the Enhanced LangGraph Health Analysis Agent with MCP Heart Attack Prediction"""
    
    print("🏥 Enhanced LangGraph Health Analysis Agent v4.0")
    print("✅ Agent is ready with Snowflake Cortex API + Interactive Chatbot + MCP Heart Attack Prediction")
    print("❤️ Heart Attack Prediction using MCP server (AdaBoost model)")
    print("🔧 To use this agent, run: streamlit run streamlit_langgraph_ui.py")
    print()
    
    # Show configuration including MCP heart attack prediction
    config = Config(
        mcp_server_url="http://localhost:8000"  # User will specify this
    )
    ...
    print()
    print("✅ Enhanced Health Agent with MCP Heart Attack Prediction ready!")
    print("🚀 Run Streamlit to start the complete health analysis workflow")
    
    return "Enhanced Agent with MCP Heart Attack Prediction ready for Streamlit integration"
