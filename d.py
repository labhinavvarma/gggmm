...
def display_enhanced_medical_extraction(structured_extractions, agent):
    """Display medical extraction with LLM code explanations - ISOLATED"""
    ...
    if medical_extraction and not medical_extraction.get('error'):
        ...
        if hlth_srvc_records:
            ...
            # Initialize explanation cache in session state (persists until page reload)
                    ...
                    try:
                        for i, record in enumerate(hlth_srvc_records):
                            ...
                            if record_key not in st.session_state.medical_explanations_cache:
                                if agent:
                                    try:
                                        # Use isolated LLM calls that don't affect chatbot context
                        ...
                        st.success("✅ All LLM explanations generated successfully! Chatbot context preserved.")
                        st.balloons()
            ...
            if medical_cache_count > 0:
                st.info(f"📋 Cached explanations: {medical_cache_count} medical records (persists until new patient)")
...
def display_enhanced_pharmacy_extraction(structured_extractions, agent):
    """Display pharmacy extraction with LLM code explanations - ISOLATED"""
    ...
    if pharmacy_extraction and not pharmacy_extraction.get('error'):
        ...
        if ndc_records:
            ...
            if pharmacy_cache_count > 0:
                st.info(f"💊 Cached explanations: {pharmacy_cache_count} pharmacy records (persists until new patient)")
...
if submitted and not st.session_state.analysis_running:
    # Clear explanation caches when new patient is analyzed (page reload simulation)
...
if st.session_state.analysis_results and not st.session_state.analysis_running:
    ...
    # 8. HEART ATTACK RISK PREDICTION BUTTON (RENAMED TO ML SERVER)
    ...
    if st.session_state.show_heart_attack:
        ...
        if heart_attack_prediction and not heart_attack_prediction.get('error'):
            ...
            st.error(f"❌ ML Server Error: {error_msg}")
            
            # Show connection info for debugging
            st.info(f"💡 Expected ML Server: {st.session_state.config.heart_attack_api_url if st.session_state.config else 'http://localhost:8080'}")
            st.info("💡 Make sure ML server is running: `python app.py`")
