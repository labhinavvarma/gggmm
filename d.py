...
def update_progress_realtime(step_name: str, step_data: Dict[str, Any], 
                           workflow_container, results_container, completed_sections: Dict[str, Any]):
    """Update progress in real-time as LangGraph processes each step"""
    
    # Update workflow step status
    for i, step in enumerate(st.session_state.workflow_steps):
        if step['name'] == step_name:
            if step_data.get('status') == 'starting':
                ...
                completed_sections[step_name] = step_data
            elif step_data.get('status') == 'error':
    ...
    # Show completed sections immediately
    with results_container:
        display_progressive_results(completed_sections)

def display_progressive_results(completed_sections: Dict[str, Any]):
    """Display results progressively as each step completes"""
    
    if not completed_sections:
        return
        
    # Mark that we've shown progressive results
    st.session_state.progressive_results_shown = True
    
    st.markdown("## 📊 Analysis Results (Live Updates)")
    st.markdown("Results appear here as each step completes...")
    
    # 1. API Fetch Results
    if 'API Fetch' in completed_sections:
        with st.expander("✅ Claims Data", expanded=True):
            st.success("✅ Claims data successfully fetched from API")
            
            data = completed_sections['API Fetch']
            if data.get('deidentified_data'):
                col1, col2 = st.columns(2)
                with col1:
                    medical_records = len(safe_get(data, 'deidentified_data.medical.medical_claims_data.records', []))
                    st.metric("Medical Records", medical_records)
                with col2:
                    pharmacy_records = len(safe_get(data, 'deidentified_data.pharmacy.pharmacy_claims_data.records', []))
                    st.metric("Pharmacy Records", pharmacy_records)
    
    # 2. Deidentification Results  
    if 'Deidentification' in completed_sections:
        with st.expander("✅ Data Privacy & Security", expanded=True):
            st.success("✅ PII removal completed - Data is now de-identified and secure")
            st.info("Patient privacy protected with advanced de-identification algorithms")
    
    # 3. Field Extraction Results
    if 'Field Extraction' in completed_sections:
        with st.expander("✅ Medical & Pharmacy Field Extraction", expanded=True):
            st.success("✅ Structured data extraction completed")
            
            data = completed_sections['Field Extraction']
            if data.get('structured_extractions'):
                display_batch_code_meanings_enhanced({'structured_extractions': data['structured_extractions']})
    
    # 4. Entity Extraction Results
    if 'Entity Extraction' in completed_sections:
        with st.expander("✅ Health Entity Analysis", expanded=True):
            st.success("✅ Health conditions and risk factors identified")
            
            data = completed_sections['Entity Extraction']
            ...
            if entity_extraction:
                ...
                for i, (col, entity) in enumerate(zip(cols, entities_data)):
                    with col:
                        value = entity['value']
                        status_class = 'positive' if entity['key'] in ['diabetics', 'smoking'] and value == 'yes' else 'negative'
    ...
    # 5. Health Trajectory Results
    if 'Health Trajectory' in completed_sections:
        with st.expander("✅ Health Trajectory Analysis", expanded=True):
            st.success("✅ Predictive health analysis completed")
            
            data = completed_sections['Health Trajectory']
    ...
    # 6. Heart Risk Prediction Results
    if 'Heart Risk Prediction' in completed_sections:
        with st.expander("✅ Cardiovascular Risk Assessment", expanded=True):
            st.success("✅ ML-based heart attack risk prediction completed")
            
            data = completed_sections['Heart Risk Prediction']
            heart_attack_prediction = data.get('heart_attack_prediction', {})
            
            if heart_attack_prediction:
                risk_score = data.get('heart_attack_risk_score', 0.0)
                risk_category = heart_attack_prediction.get('risk_category', 'Unknown')
                
                # Display risk score
                st.markdown("### ❤️ Heart Attack Risk Assessment")
                
                col1, col2, col3 = st.columns([2, 1, 2])
                with col2:
                    st.markdown(f"""
                    <div style="text-align: center; padding: 2rem; background: linear-gradient(135deg, #fff5f5 0%, #fed7d7 100%); border-radius: 15px; border: 2px solid #dc3545;">
                        <h2 style="color: #dc3545; font-size: 3rem; margin: 0;">{risk_score:.1%}</h2>
                        <p style="color: #721c24; font-weight: 600; margin: 0;">Risk Score</p>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    if 'Chatbot Initialization' in completed_sections:
        with st.expander("✅ AI Medical Assistant", expanded=True):
            st.success("✅ AI chatbot initialized and ready for interaction")
            
            # Show launch button early if chatbot is ready
            st.markdown("""
            <div class="analysis-complete-banner">
                <h3 style="margin: 0; color: #28a745;">🤖 Medical Assistant Ready!</h3>
                <p style="margin: 0.5rem 0; color: #155724;">Your AI assistant is ready to answer questions and generate visualizations.</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col2:
                if st.button(
                    ...
                    key="launch_chatbot_early",
                    use_container_width=True,
                    help="Open the dedicated Medical Assistant window"
                ):
...
def initialize_session_state():
    ...
    if 'progressive_results_shown' not in st.session_state:
        st.session_state.progressive_results_shown = False
...
if submitted:
    ...
    if not valid:
        ...
        st.session_state.progressive_results_shown = False  # Reset progressive results
        ...
        # ENHANCED WORKFLOW: Real-time sync with LangGraph progress
        with st.spinner("Running Enhanced Healthcare Analysis..."):
            try:
                ...
                st.info("🚀 Starting real-time healthcare analysis with live progress updates...")
                
                # Initialize progress tracking
                step_names = [step['name'] for step in st.session_state.workflow_steps]
                completed_sections = {}
                
                # Start the analysis with real-time step updates
                try:
                    # Check if agent supports progress callbacks
                    if hasattr(st.session_state.agent, 'run_analysis_with_progress'):
                        results = st.session_state.agent.run_analysis_with_progress(
                            patient_data,
                            progress_callback=lambda step_name, step_data: update_progress_realtime(
                                step_name, 
                                step_data, 
                                workflow_container, 
                                results_container,
...
# ENHANCED TIMING: Display workflow animation while analysis is running OR show progressive results
if st.session_state.analysis_running:
    ...
    if completed_steps > 0:
        ...
        st.success(f"✅ {completed_steps}/{total_steps} steps completed - Results appearing below in real-time!")
    else:
        st.info("🚀 Starting analysis with live progress updates...")
        
    # The workflow display and progressive results will be handled by the progress callback

# ENHANCED RESULTS SECTION - Show final summary or already displayed progressive results
if st.session_state.analysis_results and not st.session_state.analysis_running:
    results = st.session_state.analysis_results

    # Check if we already displayed results progressively
    completed_steps = sum(1 for step in st.session_state.workflow_steps if step['status'] == 'completed')
    
    if completed_steps == len(st.session_state.workflow_steps) and st.session_state.progressive_results_shown:
        # All steps completed and we showed progressive results - show final summary and chatbot launch
        st.markdown("## 🎉 Analysis Complete - Final Summary")
        st.success("✅ All analysis steps completed successfully with real-time updates shown above!")
        ...
        # Show expandable detailed sections for reference
        st.markdown("### 📋 Detailed Analysis Sections")
        st.info("💡 All sections were displayed above during processing. Expand below for detailed review:")
    
    else:
        # Some steps failed, incomplete, or no progressive results - show traditional expandable results
        if completed_steps < len(st.session_state.workflow_steps):
            st.markdown("## ⚠️ Analysis Results (Some Steps Incomplete)")
        else:
            st.markdown("## 📊 Analysis Results")
            
            # NEW: CHATBOT READY BANNER AND BUTTON for traditional flow
            if results.get("chatbot_ready", False) and st.session_state.chatbot_context:
                # Analysis Complete Banner
                st.markdown("""
                <div class="analysis-complete-banner">
                ...
                with col2:
                    if st.button(
                        "🚀 Launch Medical Assistant", 
                        key="launch_chatbot_traditional",
                        use_container_width=True,
                        help="Open the dedicated Medical Assistant window with full analysis capabilities"
                    ):
                        # Switch to chatbot page
    ...
    # Traditional expandable sections (now for detailed review or incomplete analysis)
