...
async def weather_prompt(query: str)-> List[Message]:
    return [
        {
            "role": "user",
            "content": f"""You are expert in providing weather information. You are provided with the weather_search tool to get current weather conditions, temperature, humidity, wind speed, and forecast information for any location. Use the weather_search tool to find accurate weather data. You will respond with the results returned from the weather_search tool. {query}"""
        }
    ]
...
async def web_search_prompt(query: str)-> List[Message]:
    return [
        {
            "role": "user",
            "content": f"""You are expert in finding current information from the web. You are provided with the web_search tool to search the internet and retrieve up-to-date information from multiple sources. Use the web_search tool to find the most current and relevant information available online. You will respond with the results returned from the web_search tool. {query}"""
        }
    ]
