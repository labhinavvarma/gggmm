...
with col1:
    ...
    # Load guaranteed test data
    if st.button("🧪 Load Test Data with Relationships"):
        test_data = {
            "nodes": [
        ...
        st.success("✅ Test data loaded with 4 relationships!")
        st.rerun()
    ...
    if st.session_state.conversation_history:
        ...
        st.info("No questions yet. Try the quick actions above!")
...
with col2:
    ...
    if st.session_state.graph_data:
        ...
        with col2_3:
            ...
            st.markdown(f'<div class="metric-container"><strong>{density:.1f}</strong><br>Connections</div>', unsafe_allow_html=True)
        
        # Render graph
        st.markdown("#### 🎨 Interactive Network")
        success = render_network_graph(st.session_state.graph_data)
        
        if success:
            if len(relationships) > 0:
                st.success(f"✅ Graph displayed with {len(relationships)} relationship lines!")
            else:
                st.warning("⚠️ Graph shows nodes only - no relationships found in data")
        ...
        ">
            ...
            <p><strong>This system guarantees relationship visibility</strong></p>
            <p>✨ Every query automatically includes relationships</p>
            <p>🔗 All connections are shown with colored lines</p>
            <p>🎨 Interactive graph with drag, zoom, and hover</p>
            <br>
            <p><em>Click "Load Test Data" or try a Quick Action to start!</em></p>
        </div>
        """, unsafe_allow_html=True)
...
<div style="text-align: center; color: #6c757d; padding: 0.5rem;">
    <small>🚀 <strong>Neo4j Graph Explorer</strong> | Bulletproof Edition | 
    Guaranteed Relationship Visibility</small>
</div>
""", unsafe_allow_html=True)
