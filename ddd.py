...
def display_batch_code_meanings_enhanced(results):
    """Enhanced batch processed code meanings in organized tabular format"""
    st.markdown("""
    ...
    with tab1:
        ...
        if medical_records:
            st.info(f"Found {len(unique_diagnosis_codes)} unique diagnosis codes and {len(unique_service_codes)} unique service codes")
    ...
    with tab2:
        ...
        if pharmacy_records:
            st.info(f"Found {len(unique_ndc_codes)} unique NDC codes and {len(unique_medications)} unique medications")
...
if submitted:
    ...
    if not valid:
        ...
        try:
            ...
            with st.expander("🔍 **Debug: Agent Configuration**", expanded=False):
                st.write("**Available HealthAnalysisAgent methods:**")
                available_methods = []
                for attr_name in dir(st.session_state.agent):
                    if not attr_name.startswith('_'):
                        attr = getattr(st.session_state.agent, attr_name)
                        if callable(attr):
                            available_methods.append(attr_name)
                
                st.write(f"📋 Callable methods: {', '.join(available_methods)}")
                
                # Check for LangGraph components
                if hasattr(st.session_state.agent, 'graph'):
                    st.success("✅ Agent has 'graph' attribute - LangGraph workflow ready")
                if hasattr(st.session_state.agent, 'run_analysis'):
                    st.success("✅ Agent has 'run_analysis' method - Main workflow method found")
                if hasattr(st.session_state.agent, 'chat_with_data'):
                    st.success("✅ Agent has 'chat_with_data' method - Chatbot functionality ready")
                if hasattr(st.session_state.agent, 'run_analysis_with_progress'):
                    st.success("✅ Agent has 'run_analysis_with_progress' method - Real-time progress available")
                else:
                    st.info("ℹ️ Agent uses standard 'run_analysis' method - Progress simulation will be used")
                
                # Show configuration
                st.write("**Agent Configuration:**")
                st.write(f"• Snowflake API URL: {config.api_url}")
                st.write(f"• Model: {config.model}")
                st.write(f"• Heart Attack API: {config.heart_attack_api_url}")
                st.write(f"• FastAPI URL: {config.fastapi_url}")
        ...
        # Create containers for workflow display
        ...
        # ENHANCED WORKFLOW: Call actual LangGraph
        with st.spinner("🚀 Running Enhanced Healthcare Analysis with LangGraph..."):
            try:
                st.info("📍 Executing comprehensive LangGraph workflow analysis...")
                
                # Show visual progress simulation
                ...
                # NOW CALL THE ACTUAL LANGGRAPH WORKFLOW
                st.info("🧠 Executing LangGraph nodes and processing comprehensive healthcare data...")
                
                # This is the key fix - call the actual LangGraph workflow
                ...
                # Process results
                analysis_success = results.get("success", False) if results else False
                
                if analysis_success:
                    st.success("✅ LangGraph workflow analysis completed successfully!")
                    st.balloons()
                    
                    with results_container:
                        st.success("✅ All LangGraph nodes executed successfully!")
                        
                        # Show key results
                        if results.get('chatbot_ready'):
                            st.info("💬 Enhanced chatbot with graph generation is ready!")
                        
                        if results.get('errors'):
                            st.warning("⚠️ Some non-critical issues occurred:")
                    ...
                    with results_container:
                        st.error("❌ LangGraph analysis failed!")
                        ...
                        # Mark all steps as error if analysis failed
                ...
                with results_container:
                    ...
                    st.error("Please check your agent configuration and try again.")

# Display workflow animation while analysis is running
if st.session_state.analysis_running:
    ...
    if completed_steps > 0:
        st.info(f"✅ {completed_steps}/{total_steps} steps completed")
    else:
        st.info("🚀 Starting LangGraph workflow analysis...")
...
if st.session_state.analysis_results and not st.session_state.analysis_running:
    ...
    if results.get("success"):
        ...
        # Chatbot launch button if ready
    ...
    with st.expander("❤️ Heart Attack Risk Prediction", expanded=False):
        ...
        if heart_attack_prediction and not heart_attack_prediction.get('error'):
            ...
            if risk_percentage:
                ...
                st.markdown("""
                <div style="text-align: center; margin: 2rem 0;">
                    <h3 style="color: #2c3e50;">Heart Disease Risk</h3>
