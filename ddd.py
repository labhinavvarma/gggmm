...
async def serpapi_prompt(query: str)-> List[Message]:
    ...
    return [
        {
            "role": "user", 
            "content": f"""You are a web search expert. You have access to the `SerpApiSearch` tool which automatically detects when queries need recent information and applies appropriate filters.

            For getting current information:
            - Include current year in queries when relevant
            - Use terms like "current", "latest", "recent", "today" for time-sensitive topics
            - Be specific about timeframes when searching for recent events

            The SerpApiSearch tool will automatically:
            - Apply recent time filters for current event queries
            - Use Google News search for news topics
            ...
            Your task: Search for information about: {query}

            Provide comprehensive search results based exactly on what the search returns."""
        }
    ]
