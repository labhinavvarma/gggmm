...
class HealthAnalysisAgent:
    ...
    def _handle_heart_attack_question(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Handle heart attack related questions with simplified prompts"""
        try:
            # Get prediction data
            heart_attack_prediction = chat_context.get("heart_attack_prediction", {})
            entity_extraction = chat_context.get("entity_extraction", {})
            
            # Extract simple text summaries
            ...
            if chat_history:
                recent_history = chat_history[-3:]  # Limit to 3 recent messages
                history_lines = []
            ...
            # Create simple prompt without complex JSON
            heart_attack_prompt = f"""You are a cardiologist analyzing heart attack risk.
...
Provide a cardiovascular risk assessment with:
- Risk Percentage
...
Use simple language and avoid technical jargon."""
            ...
            logger.info(f"Processing heart attack question: {user_query[:50]}...")
            
            # Simple system message
            system_msg = "You are a cardiologist providing heart disease risk analysis. Use clear, simple language."
            
            response = self.api_integrator.call_llm(heart_attack_prompt, system_msg)
            
            if response.startswith("Error"):
                return "I encountered an error analyzing heart attack risk. Please try rephrasing your question."
            
            return response
            
        except Exception as e:
            logger.error(f"Error in heart attack question: {str(e)}")
            return "I encountered an error. Please try again with a simpler question about heart disease risk."

    def _handle_general_question(self, user_query: str, chat_context: Dict[str, Any], chat_history: List[Dict[str, str]]) -> str:
        """Handle general questions with simplified context"""
        try:
            # Extract simple summaries instead of full JSON
            medical_summary = self._extract_simple_medical_summary(chat_context)
            pharmacy_summary = self._extract_simple_pharmacy_summary(chat_context)
            ...
            simple_prompt = f"""You are a medical assistant with access to patient claims data.
...
Answer the question using the available medical and pharmacy data. Be specific about dates, codes, and medications when relevant. Use simple, clear language."""

            logger.info(f"Processing general query: {user_query[:50]}...")
            
            # Simple system message
            system_msg = "You are a healthcare assistant. Provide clear, accurate information based on the patient data."
            
            response = self.api_integrator.call_llm(simple_prompt, system_msg)
    ...
    def _create_comprehensive_trajectory_prompt(self, medical_data: Dict, pharmacy_data: Dict, mcid_data: Dict,
                                              ...
                                              entities: Dict) -> str:
        """Create simplified prompt for health trajectory analysis"""
        
        # Extract simple text summaries instead of embedding JSON
        ...
        entity_summary = self._extract_entity_summary(entities)
        
        return f"""You are a healthcare AI assistant analyzing a patient's health trajectory. 
...
Provide a detailed analysis in 500-600 words."""
    ...
    def _create_comprehensive_summary_prompt(self, trajectory_analysis: str, entities: Dict, 
        ...
        """Create simplified prompt for final health summary"""
        
        # Simple text extraction instead of JSON
        diabetes_status = entities.get('diabetics', 'unknown')
        smoking_status = entities.get('smoking', 'unknown')
        bp_status = entities.get('blood_pressure', 'unknown')
        
        medical_record_count = len(medical_extraction.get('hlth_srvc_records', []))
        pharmacy_record_count = len(pharmacy_extraction.get('ndc_records', []))
        
        return f"""Based on the health trajectory analysis below, create a concise executive summary.
...
- Medical Records: {medical_record_count}
- Pharmacy Records: {pharmacy_record_count}
...
Keep under 300 words and focus on actionable insights."""

    # Helper methods for extracting simple text summaries

    def _extract_medical_summary(self, medical_data: Dict, medical_extraction: Dict) -> str:
        """Extract simple text summary from medical data"""
        try:
            summary_parts = []
            ...
            # Recent diagnoses (limit to avoid JSON issues)
            if records:
                recent_diagnoses = []
                for record in records[:5]:  # Limit to 5 records
                    diag_code = record.get('diag_cd', 'Unknown')
                    service_date = record.get('clm_rcvd_dt', 'Unknown date')
                    recent_diagnoses.append(f"Diagnosis {diag_code} on {service_date}")
    ...
    def _extract_pharmacy_summary(self, pharmacy_data: Dict, pharmacy_extraction: Dict) -> str:
        """Extract simple text summary from pharmacy data"""
        try:
            summary_parts = []
            
            # Pharmacy records count
            records = pharmacy_extraction.get('ndc_records', [])
            summary_parts.append(f"Pharmacy Records: {len(records)} medication records")
            
            # Recent medications (limit to avoid JSON issues)
            if records:
                recent_meds = []
                for record in records[:5]:  # Limit to 5 records
                    ndc_code = record.get('ndc_cd', 'Unknown')
                    label_name = record.get('label_name', 'Unknown medication')
    ...
    def _extract_entity_summary(self, entities: Dict) -> str:
        """Extract simple text summary from entities"""
        try:
            summary_parts = []
            ...
            if conditions:
                summary_parts.append(f"Medical Conditions: {', '.join(conditions[:5])}")  # Limit to 5
            
            # Medications
            medications = entities.get('medications_identified', [])
            if medications:
                summary_parts.append(f"Medications: {', '.join(medications[:5])}")  # Limit to 5
            ...
            summary_parts.append(f"Risk Factors - Diabetes: {diabetes}, Smoking: {smoking}, Blood Pressure: {bp}")
            
            return "\n".join(summary_parts)
            
        except Exception as e:
            return f"Health entities available but summary extraction failed: {str(e)}"

    def _extract_simple_medical_summary(self, chat_context: Dict[str, Any]) -> str:
        """Extract simple medical summary for general questions"""
        try:
            deidentified_medical = chat_context.get("deidentified_medical", {})
            medical_extraction = chat_context.get("medical_extraction", {})
            
            age = deidentified_medical.get("src_mbr_age", "unknown")
            records_count = len(medical_extraction.get('hlth_srvc_records', []))
            
            return f"Patient age {age} with {records_count} medical service records available"
            
        except Exception as e:
            return "Medical data available"

    def _extract_simple_pharmacy_summary(self, chat_context: Dict[str, Any]) -> str:
        """Extract simple pharmacy summary for general questions"""
        try:
            pharmacy_extraction = chat_context.get("pharmacy_extraction", {})
            records_count = len(pharmacy_extraction.get('ndc_records', []))
            
            return f"{records_count} pharmacy records with medication details available"
            
        except Exception as e:
            return "Pharmacy data available"
