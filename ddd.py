...
st.sidebar.success("🔍 SerpApi Configured on Server")
st.sidebar.caption("🔐 Authentication handled via structured requests")
st.sidebar.caption("🌐 Uses JSON endpoint with request configuration")
...
if show_server_info:
 ...
 # Display Prompts
    ...
    if prompt_type == "Web Search":
        st.sidebar.info("🌐 Web Search uses SerpApi structured HTTP requests")
        st.sidebar.caption("💡 Tip: For current events, include '2025', 'current', 'latest', or 'recent'")
        st.sidebar.caption("🔧 Request configuration handled automatically")
        debug_raw = st.sidebar.checkbox("🔧 Show Raw JSON Response (Debug)", value=False)
        url_test = st.sidebar.checkbox("🔗 Show Request Configuration (Test)", value=False)
        if debug_raw:
            st.sidebar.warning("⚠️ Debug mode shows complete raw JSON response")
        if url_test:
            st.sidebar.info("🔗 Test mode shows request configuration details")
    else:
        debug_raw = False
        url_test = False
        ...
        async def process_query(query_text):
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    # If using Web Search, create appropriate prompts based on mode
                    if prompt_type == "Web Search":
                        if url_test:
                            # Use the URL test tool to show request configuration
                            prompt_from_server = [
                                {
                                    "role": "user",
                                    "content": f"""🔗 REQUEST CONFIGURATION TEST MODE 🔗

                                    You MUST use the SerpApiUrlTest tool to show the exact request configuration.

                                    MANDATORY STEPS:
                                    1. Call SerpApiUrlTest tool RIGHT NOW with:
                                       - query: "{query_text}"
                                    
                                    2. Show me the request configuration and constructed URL
                                    
                                    3. Explain what parameters and filters are being applied

                                    CALL THE TOOL NOW - This shows how requests are structured."""
                                }
                            ]
                        elif debug_raw:
                            # Use the debug tool to show raw JSON response
                            prompt_from_server = [
                                {
                                    "role": "user",
                                    "content": f"""🔧 RAW DEBUG MODE ACTIVATED 🔧

                                    You MUST use the SerpApiRawDebug tool to show the complete JSON response.

                                    MANDATORY STEPS:
                                    1. Call SerpApiRawDebug tool RIGHT NOW with:
                                       - query: "{query_text}"
                                    
                                    2. Show me the complete raw JSON response
                                    
                                    3. Do NOT interpret the results - just display the raw data

                                    CALL THE TOOL NOW - This is for debugging the JSON response."""
                                }
                            ]
                        else:
                            # Normal search mode
                            prompt_from_server = [
                                {
                                    ...
                                    MANDATORY STEPS:
                                    1. IMMEDIATELY call SerpApiSearch tool with:
                                       - query: "{query_text}"
                                    
                                    2. After getting search results, provide a clear answer based on what you found
                                    
                                    3. Cite the sources when providing information

                                    DO NOT provide any response without first calling the SerpApiSearch tool. 
                                    The tool call is mandatory. Authentication is handled automatically."""
                                }
                            ]
