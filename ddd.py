        ...
        async def process_query(query_text):
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    if prompt_type == "Web Search" and web_search_available:
                        ...
                        try:
                            ...
                            if web_search_tool:
                                ...
                                try:
                                    # Try the actual tool function call
                                    test_result = await web_search_tool.func(query=query_text, limit=3)
                    ...
                    # Add more detailed agent monitoring
                    class DebugAgent:
                        ...
                        async def ainvoke(self, input_data):
                            st.write("🔍 **Agent starting processing...**")
                            response = await self.original_agent.ainvoke(input_data)
                            return response
                    
                    # Wrap agent for debugging if it's web search
                    if prompt_type == "Web Search":
                        debug_agent = DebugAgent(agent)
                        response = await debug_agent.ainvoke({"messages": prompt_from_server})
                    else:
