...
"""
    ...
    def generate_risk_dashboard(self, chat_context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Extract feature interpretations safely
            feature_interpretation = heart_attack_features.get('feature_interpretation', {})
            age_text = feature_interpretation.get('Age', '50 years old')
            gender_text = feature_interpretation.get('Gender', 'Unknown')
            diabetes_text = feature_interpretation.get('Diabetes', 'Unknown')
            bp_text = feature_interpretation.get('High_BP', 'Unknown')
            smoking_text = feature_interpretation.get('Smoking', 'Unknown')
...
# Extract age from text safely
try:
    age_value = int(age_text.split()[0]) if age_text.split() else 50
except:
    age_value = 50

risk_values = [
    age_value / 100,
    1 if 'male' in gender_text.lower() else 0,
    1 if diabetes_text == 'Yes' else 0,
    1 if bp_text == 'Yes' else 0,
    1 if smoking_text == 'Yes' else 0
]
...
if bp_status in ['managed', 'diagnosed']:
...
med_values = [medications_count, max(1, medications_count//3), max(1, medications_count//4)]
...
conditions_count = len([x for x in [diabetes_status, bp_status, smoking_status] if x in ['yes', 'managed', 'diagnosed']])
...
MEDICATIONS: {{medications_count}} identified
CONDITIONS: {{conditions_count}} active
...
- **Primary Risk Factors:** {', '.join([k for k, v in feature_interpretation.items() if v == 'Yes'])}
- **Medications Identified:** {medications_count} therapeutic agents
- **Active Conditions:** {len([x for x in [diabetes_status, bp_status, smoking_status] if x in ['yes', 'managed', 'diagnosed']])} conditions requiring monitoring
...
"""
    ...
    def generate_comprehensive_health_overview(self, chat_context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Pre-process data for matplotlib
            medical_records_count = len(medical_extraction.get('hlth_srvc_records', []))
            pharmacy_records_count = len(pharmacy_extraction.get('ndc_records', []))
            heart_risk_score = heart_attack_prediction.get('raw_risk_score', 0.15) * 100
            
            # Extract key values safely
            diabetes_status = entity_extraction.get('diabetics', 'unknown')
            bp_status = entity_extraction.get('blood_pressure', 'unknown')
            smoking_status = entity_extraction.get('smoking', 'unknown')
            patient_age = entity_extraction.get('age', 50)
            medical_conditions_count = len(entity_extraction.get('medical_conditions', []))
            medications_count = len(entity_extraction.get('medications_identified', []))
...
# Data extraction - safely defined
medical_records = {medical_records_count}
pharmacy_records = {pharmacy_records_count}
heart_risk = {heart_risk_score}

# Entity data - safely extracted
diabetes_status = '{diabetes_status}'
bp_status = '{bp_status}'
smoking_status = '{smoking_status}'
patient_age = {patient_age if isinstance(patient_age, (int, float)) else 50}
...
📊 Total Conditions: {{medical_conditions_count}}
...
values = [medical_records, pharmacy_records, medical_conditions_count, medications_count]
...
age_risk = min(1.0, patient_age / 80)  # Scale age risk

values = [
    age_risk,
    0.6,  # Default gender risk
    1 if diabetes_status == 'yes' else 0,
    1 if bp_status in ['managed', 'diagnosed'] else 0,
...
# Mock severity data (0-3 scale) - safely calculated
diabetes_severity = 2 if diabetes_status == 'yes' else 0
bp_severity = 2 if bp_status in ['managed', 'diagnosed'] else 0
smoking_severity = 3 if smoking_status == 'yes' else 0
cardio_severity = int(heart_risk/25)

severity_data = np.array([
    [diabetes_severity, diabetes_severity, min(3, diabetes_severity + 1)],
    [bp_severity, bp_severity, bp_severity],
    [smoking_severity, smoking_severity, max(0, smoking_severity - 1)],
    [cardio_severity, cardio_severity, min(3, cardio_severity + 1)],
    [1, 1, 1]
])
...
**Key Health Insights:**
- **Data Completeness:** {medical_records_count} medical + {pharmacy_records_count} pharmacy records
- **Risk Assessment:** {heart_attack_prediction.get('risk_display', 'Pending assessment')}
- **Active Conditions:** {medical_conditions_count} identified health conditions
- **Current Medications:** {medications_count} therapeutic agents
    ...
    def _execute_matplotlib_code(self, code: str) -> Optional[io.BytesIO]:
        ...
        try:
            ...
            namespace = {
                ...
                'patient_age': 50,
                'heart_risk_score': 15.0,
                'medications_count': 3,
                'medical_conditions_count': 2
            }
            ...
            logger.error(f"Undefined variable in matplotlib code: {e}")
            return None
