...
# Enhanced CSS with WIDER SIDEBAR when chatbot is active
st.markdown("""
<style>
...
/* ENHANCED SIDEBAR SIZING - Make sidebar wider when chatbot is active */
.css-1d391kg {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
}

/* Wide sidebar when chatbot is ready */
.chatbot-active .css-1d391kg {
    width: 50% !important;
    min-width: 600px !important;
}

/* Adjust main content when sidebar is wide */
.chatbot-active .css-k1vhr4 {
...
/* Chat message containers in sidebar */
.stChatMessage {
...
/* ENHANCED WORKFLOW ANIMATION - NOW SHOWS IMMEDIATELY */
.advanced-workflow-container {
    ...
    position: relative;
    overflow: hidden;
    animation: workflow-entrance 0.8s ease-out;
}

@keyframes workflow-entrance {
    0% { 
        opacity: 0;
...
.immediate-loading-graphics {
    ...
    margin: 2rem 0;
    border: 2px solid #2196f3;
    animation: immediate-entrance 0.5s ease-out;
}

@keyframes immediate-entrance {
    0% { 
        opacity: 0;
...
# Apply chatbot active class when chatbot is ready
if st.session_state.get('analysis_results') and st.session_state.analysis_results.get("chatbot_ready", False):
    st.markdown('<script>document.body.classList.add("chatbot-active");</script>', unsafe_allow_html=True)
...
def initialize_session_state():
    ...
    if 'workflow_placeholder' not in st.session_state:
        st.session_state.workflow_placeholder = None
...
def display_immediate_loading_graphics():
    """Display immediate loading graphics when analysis button is clicked"""
    st.markdown("""
    <div class="immediate-loading-graphics">
        <div style="text-align: center;">
            <h3>🚀 Initializing Healthcare Analysis Pipeline</h3>
            <div class="loading-spinner" style="margin: 2rem auto;"></div>
            <p>Preparing comprehensive analysis with graph generation capabilities...</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    # Show interactive loading graphs
    try:
        loading_fig = create_chatbot_loading_graphs()
        st.plotly_chart(loading_fig, use_container_width=True, key="immediate_loading_graphs")
    except Exception as e:
        st.info("📊 Health analytics dashboard initializing...")
...
def display_batch_code_meanings_enhanced(results):
    ...
    with tab1:
        ...
        with med_tab1:
            ...
            if medical_records:
                # Prepare data for enhanced table display - FIXED TO INCLUDE ALL CODES
                diagnosis_data = []
                all_diagnosis_codes = set()
                
                # Collect all diagnosis codes from records
                for record in medical_records:
                    ...
                    for diag in record.get("diagnosis_codes", []):
                        code = diag.get("code", "")
                        if code:
                            all_diagnosis_codes.add(code)
                            # Get meaning from batch results OR create fallback
                ...
                if diagnosis_data:
                    ...
                    unique_codes = len(all_diagnosis_codes)
                    st.info(f"📊 **Total ICD-10 Codes Found:** {unique_codes} | **Meanings Available:** {len(diagnosis_meanings)}")
                    ...
                    st.success(f"📊 Successfully processed {len(diagnosis_data)} diagnosis code entries")
                    ...
                    with st.expander("📈 ICD-10 Code Frequency Analysis"):
                        ...
                        for code, count in code_counts.head(5).items():
                            meaning = diagnosis_meanings.get(code, f"ICD-10 diagnosis code: {code}")
        ...
        with med_tab2:
            ...
            if medical_records:
                ...
                for record in medical_records:
                    ...
                    if service_code:
                        all_service_codes.add(service_code)
                        # Get meaning from batch results OR create fallback
                ...
                if service_data:
                    # Display unique code count
                    unique_codes = len(all_service_codes)
                    st.info(f"📊 **Total Service Codes Found:** {unique_codes} | **Meanings Available:** {len(service_meanings)}")
                    ...
                    st.success(f"📊 Successfully processed {len(service_data)} service code entries")
                    ...
                    with st.expander("📈 Service Code Frequency Analysis"):
                        ...
                        for code, count in code_counts.head(5).items():
                            meaning = service_meanings.get(code, f"Medical service code: {code}")
                            st.write(f"• **{code}** ({count}x): {meaning}")
                else:
    ...
    with tab2:
        ...
        with pharm_tab1:
            ...
            if pharmacy_records:
                ...
                if ndc_data:
                    # Display unique code count
                    unique_codes = len(all_ndc_codes)
                    st.info(f"📊 **Total NDC Codes Found:** {unique_codes} | **Meanings Available:** {len(ndc_meanings)}")
                    ...
                    st.success(f"📊 Successfully processed {len(ndc_data)} NDC code entries")
                    ...
                    with st.expander("📈 NDC Code Frequency Analysis"):
                        ...
                        for code, count in code_counts.head(5).items():
                            meaning = ndc_meanings.get(code, f"NDC medication code: {code}")
                            st.write(f"• **{code}** ({count}x): {meaning}")
                else:
        ...
        with pharm_tab2:
            ...
            if pharmacy_records:
                # Prepare data for enhanced table display - FIXED TO INCLUDE ALL MEDICATIONS
                medication_data = []
                all_medications = set()
                ...
                if medication_data:
                    ...
                    unique_meds = len(all_medications)
                    st.info(f"📊 **Total Medications Found:** {unique_meds} | **Meanings Available:** {len(med_meanings)}")
                    ...
                    st.success(f"📊 Successfully processed {len(medication_data)} medication entries")
...
if submitted:
    ...
    if not valid:
        ...
        # FIXED: Show immediate loading graphics as soon as button is clicked
        st.session_state.workflow_placeholder = st.empty()
        with st.session_state.workflow_placeholder.container():
            display_immediate_loading_graphics()
        ...
        with st.spinner("🔬 Running Enhanced Healthcare Analysis..."):
            try:
                # Now show the workflow animation immediately
                with st.session_state.workflow_placeholder.container():
...
if st.session_state.analysis_running and st.session_state.workflow_placeholder:
    with st.session_state.workflow_placeholder.container():
        display_advanced_professional_workflow()
...
if st.session_state.analysis_results and not st.session_state.analysis_running:
    ...
    if st.session_state.show_heart_attack:
        ...
        if heart_attack_prediction and not heart_attack_prediction.get('error'):
            ...
            st.warning("⚠️ Heart attack prediction not available or encountered errors.")
