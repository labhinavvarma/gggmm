...
def start_mcp_server():
    ...
    try:
        st.info("🚀 Starting Neo4j MCP Server...")
...
def ensure_mcp_server():
    ...
    if "mcp_initialized" not in st.session_state:
        ...
        max_wait = 10  # seconds
        wait_time = 0
        with st.spinner("⏳ Waiting for MCP server to start..."):
            while wait_time < max_wait and not _mcp_server_ready:
                time.sleep(0.5)
                wait_time += 0.5
        
        if not _mcp_server_ready:
            st.error("❌ MCP server failed to start within timeout")
            return False
        
        # Test connection
        if test_mcp_connection():
            st.success("🎉 MCP Server connected and ready!")
            return True
        else:
            st.error("❌ MCP server started but connection test failed")
            return False
