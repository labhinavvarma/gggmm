    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if fallback_results:
                ...
                result += f"1. **{fallback_results[0].title}**\n"
                result += f"   {fallback_results[0].description}\n"
                result += f"   🔗 {fallback_results[0].url}\n"
                return result
...
# Test Tool Prompt for ChatSnowflakeCortex Custom Format
@mcp.prompt(
...
async def test_tool_prompt(message: str = "connectivity test") -> List[Message]:
    """
    Test prompt using ChatSnowflakeCortex custom tool format.
    
    Args:
        message: Test message to send
    
    Returns:
        Formatted prompt messages for ChatSnowflakeCortex tool calling
    """
    return [
        {
            "role": "system",
            "content": {
                "invoke_tool": json.dumps({
                    "tool_name": "test_tool",
                    "args": {
                        "message": message
                    }
        ...
        {
            "role": "user",
            "content": f"Please execute the test_tool with message: {message}"
        }
    ]

# Diagnostic Prompt for ChatSnowflakeCortex Custom Format  
@mcp.prompt(
...
async def diagnostic_prompt(test_type: str = "basic") -> List[Message]:
    """
    Diagnostic prompt using ChatSnowflakeCortex custom tool format.
    ...
    return [
        ...
        {
            "role": "user",
            "content": f"Please run diagnostic test: {test_type}"
        }
    ]

# Custom Format Web Search Prompt for ChatSnowflakeCortex
@mcp.prompt(
...
async def web_search_analysis_prompt(query: str, results_count: int = 5) -> List[Message]:
    """
    Generate a prompt that works with ChatSnowflakeCortex custom tool format.
    ...
    return [
        ...
        {
            "role": "user",
            "content": f"""Based on the web search results for "{query}", please provide current information about this topic. 

Focus on the most recent and accurate information from the search results. If the search results seem outdated, please mention that and provide what information is available.
