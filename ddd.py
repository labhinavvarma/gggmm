...
@mcp.tool(
       ...
       ,description="""
        ...
        This tool returns the literal JSON response without any interpretation.
...
async def serpapi_raw_debug(query: str, api_key: str) -> str:
    """Debug tool that returns raw SerpApi JSON response and prints to server logs"""
    ...
    try:
        ...
        print(f"{'='*80}")
        print(f"PARAMETERS SENT TO SERPAPI:")
...
@mcp.tool(
       ...
       ,description="""
        ...
        Use debug_raw=true to see the complete JSON response from SerpApi.
        ...
        Args:
               ...
               debug_raw: If true, returns complete raw JSON response from SerpApi (default: false)
        """
)
async def serpapi_search(query: str, api_key: str, location: str = "", num_results: int = 5, debug_raw = False) -> str:
    """Tool to perform web searches using SerpApi, returns raw search results for LLM interpretation"""
    
    import json
    
    print(f"\n🔍 TOOL INVOKED: SerpApiSearch")
    print(f"📞 CALL TIME: {json.dumps({'query': query, 'debug_raw': debug_raw, 'timestamp': 'now'})}")
    ...
    # Convert debug_raw to boolean if it's a string
    if isinstance(debug_raw, str):
        debug_raw = debug_raw.lower() in ('true', '1', 'yes', 'on')
    
    print(f"🔧 DEBUG_RAW MODE: {debug_raw}")
    ...
    try:
        ...
        logger.info(f"SerpApi search for '{query}' with debug_raw={debug_raw}")
        logger.info(f"Search params: {params}")
        
        # If debug_raw is True, return the complete raw JSON response
        if debug_raw:
            raw_json = json.dumps(results, indent=2, ensure_ascii=False)
            logger.info(f"Returning raw JSON (length: {len(raw_json)} chars)")
            print(f"🔧 DEBUG MODE: Returning raw JSON response")
            return f"🔧 RAW SERPAPI RESPONSE FOR QUERY: '{query}'\n\n{raw_json}"
        ...
        search_info += "Use debug_raw=true parameter to see complete raw JSON response.\n"
