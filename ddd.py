...
if show_server_info:
    ...
    with st.sidebar.expander("🛠 Tools", expanded=False):
        tool_categories = {
            ...
            "Search & Web": ["debug_web_scraping", "block_old_data_responses", "mandatory_web_search", "verify_web_scraping", "force_current_search", "search_and_analyze", "duckduckgo_search", "fetch_content", "web_search", "real_search"],
            "Weather": ["get_weather"],
    ...
    examples = {
        ...
        "Search Expert": [
            "Use block_old_data_responses to find the latest news about artificial intelligence and summarize what's happening right now",
            "Use block_old_data_responses to search for current climate change impacts in 2024 and tell me what's happening this year", 
            "Use block_old_data_responses to find recent renewable energy technology developments and explain the latest innovations",
            "Use block_old_data_responses to search for today's stock market trends and analyze current market conditions",
            "Use block_old_data_responses to find current machine learning healthcare applications and recent breakthroughs",
            "Use block_old_data_responses to research latest quantum computing developments and explain newest breakthroughs",
            "Use block_old_data_responses to find current electric vehicle sales data and adoption rates this year",
            "🔧 DEBUG: Use debug_web_scraping to diagnose why web scraping might be failing",
            "🔧 TEST: Use verify_web_scraping to test if web scraping tools are working properly"
        ],
        "No Context": [
        ...
        async def process_query(query_text):
            with st.chat_message("assistant"):
                ...
                progress_steps = [
                    ...
                    "🛠️ Loading data blocking and web scraping tools...",
                    "🧠 Creating intelligent agent...",
                    "📝 Loading expert prompt with security protocol...",
                    "🔒 Activating training data blocker and web scraper...",
                    "📄 Scraping live website content...",
                    "✍️ Generating response from current web data..."
                ]
                ...
                try:
                    ...
                    search_tools = [t for t in tools if 'search' in t.name.lower() or 'scraping' in t.name.lower() or 'mandatory' in t.name.lower() or 'block' in t.name.lower()]
                    if search_tools:
                    ...
                    if prompt_type == "Search Expert":
                        # Look for specific indicators that web scraping actually occurred
                        scraping_indicators = [
                            "OLD DATA BLOCK REMOVED",
                            "MANDATORY WEB SEARCH COMPLETED",
                            "LIVE SCRAPED CONTENT", 
                            "SCRAPED WEBSITE CONTENT",
                            "Training data access blocked",
                        ...
                        scraping_detected = any(indicator in result for indicator in scraping_indicators)
                        failure_detected = any(indicator in result for indicator in failure_indicators)
                        
                        if scraping_detected and not failure_detected:
                            result = f"✅ **LIVE WEB SCRAPING SUCCESSFUL** ✅\n\n{result}"
                        elif failure_detected:
                            result = f"""🚨 **WEB SCRAPING PIPELINE FAILURE DETECTED** 🚨

The old data blocking worked, but web scraping failed to retrieve current content.

**🔧 DIAGNOSIS NEEDED:**
The system found search results but couldn't scrape website content. This usually means:
- Websites are blocking our scraper (most common)
- Bot detection systems are active
- JavaScript-required content (our scraper can't run JS)
- Geographic content restrictions
- Rate limiting or timeouts

**🛠️ IMMEDIATE TROUBLESHOOTING:**
1. Try: "Use debug_web_scraping to diagnose scraping issues with [your topic]"
2. Use the "🔧 Full Debug" button in the sidebar
3. Check if the MCP server has internet access
4. Try different search terms or topics

**🔄 ALTERNATIVE APPROACHES:**
- Try searching for more general terms
- Look for different types of websites
- Use academic or news sources instead of commercial sites
...
🔧 **To fix this, run the debug tool to see exactly where the scraping is failing.**"""
                        else:
                            # No indicators found - possible training data leak
                            result = f"""🚨 **CRITICAL: TRAINING DATA LEAK DETECTED** 🚨

The response below is using OLD TRAINING DATA instead of current web information!

**🔒 DATA SECURITY VIOLATION:**
- The blocking system was bypassed
- Web scraping tools were not used
- Response contains potentially outdated information

**🛠️ IMMEDIATE ACTIONS REQUIRED:**
1. Try asking: "Use block_old_data_responses to find current information about [your topic]"
2. Use the diagnostic tools in the sidebar
3. Verify your MCP server is properly configured

**🔧 Alternative Commands:**
- "Use debug_web_scraping to diagnose issues with [topic]"
- "Use mandatory_web_search to..."
- "Use force_current_search to..."

---

⚠️ **POTENTIALLY OUTDATED RESPONSE BELOW** ⚠️

{result}

---

🚫 **This information may be from training data. Request new web search for current data.**"""
