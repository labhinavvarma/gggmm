        ...
        async with sse_client(MCP_URL) as sse:
            async with ClientSession(*sse) as session:
                ...
                if tool_name == "health_check":
                    # Health check can take optional query parameter
                    result = await session.call_tool(tool.name, {"query": query} if query else {})
                elif tool_name == "get_database_schema":
                    # Schema tool doesn't need query parameter
                    result = await session.call_tool(tool.name, {})
                else:
                    # Other tools need query parameter
                ...
                if result and result.content:
                    ...
                    return "✅ Tool executed successfully (no results returned)"
            ...
            with st.expander("💡 Example Queries"):
                ...
                with col1:
                    if st.button("📱 Popular Apps"):
                        st.session_state.history.append(("user", "Show me the top 10 most popular apps"))
                        st.rerun()
                    if st.button("🏃 Fitness Apps"):
                        st.session_state.history.append(("user", "What are the highest rated fitness apps?"))
                        st.rerun()
                with col2:
                    if st.button("📊 Device Compatibility"):
                        st.session_state.history.append(("user", "Which apps are compatible with Garmin devices?"))
                        st.rerun()
                    if st.button("👨‍💻 Top Developers"):
                        st.session_state.history.append(("user", "Show me apps by the most successful developers"))
                        st.rerun()
            
            # Chat input
            user_query = st.chat_input("Ask me anything about your Neo4j ConnectIQ database...")
