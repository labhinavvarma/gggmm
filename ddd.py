...
# Determine sidebar state - EXPANDED by default for better chat experience
...
# Enhanced CSS with advanced animations and modern styling + new sections + WIDER SIDEBAR
st.markdown("""
<style>
...
/* EXPANDED SIDEBAR WIDTH FOR BETTER CHAT EXPERIENCE */
.css-1d391kg {
...
# Initialize session state - REMOVED TOGGLE VARIABLES
...
def display_batch_code_meanings_enhanced(results):
    """Enhanced batch processed code meanings in organized tabular format with proper subdivisions and FIXED NDC METRICS"""
    st.markdown("""
    ...
    with tab2:
        ...
        # FIXED: Better extraction of pharmacy meanings with multiple fallback paths
        pharmacy_meanings = pharmacy_extraction.get("code_meanings", {})
        
        # Try multiple possible structures for NDC meanings
        ndc_meanings = (
            pharmacy_meanings.get("ndc_code_meanings") or 
            pharmacy_meanings.get("ndc_meanings") or 
            pharmacy_extraction.get("ndc_code_meanings") or
            pharmacy_extraction.get("ndc_meanings") or
            {}
        )
        
        # Try multiple possible structures for medication meanings  
        med_meanings = (
            pharmacy_meanings.get("medication_meanings") or
            pharmacy_meanings.get("med_meanings") or
            pharmacy_extraction.get("medication_meanings") or
            pharmacy_extraction.get("med_meanings") or
            {}
        )
        ...
        st.write("**Debug Info:**")
        st.write(f"- Pharmacy extraction keys: {list(pharmacy_extraction.keys())}")
        st.write(f"- Code meanings keys: {list(pharmacy_meanings.keys())}")
        st.write(f"- NDC meanings count: {len(ndc_meanings)}")
        st.write(f"- Med meanings count: {len(med_meanings)}")
        st.write(f"- Pharmacy records count: {len(pharmacy_records)}")
        
        # Show sample of what's available
        if ndc_meanings:
            st.write(f"- Sample NDC meanings: {list(ndc_meanings.items())[:3]}")
        if med_meanings:
            st.write(f"- Sample med meanings: {list(med_meanings.items())[:3]}")
        ...
        with pharm_tab1:
            ...
            if pharmacy_records:
                ...
                for record in pharmacy_records:
                    ...
                    if ndc_code:
                        # FIXED: Better NDC meaning lookup with enhanced fallback
                        ndc_meaning = (
                            ndc_meanings.get(ndc_code) or
                            ndc_meanings.get(ndc_code.replace('-', '')) or  # Try without dashes
                            ndc_meanings.get(ndc_code.replace('-', '')[:9]) or  # Try first 9 digits
                            f"NDC {ndc_code} - {label_name}" if label_name else f"NDC {ndc_code}"
                        )
                ...
                if ndc_data:
                    ...
                    # Show how many have actual meanings vs fallback
                    meaningful_count = sum(1 for item in ndc_data if not item["NDC Meaning"].startswith("NDC "))
                    st.info(f"📋 **NDC Codes with LLM Meanings:** {meaningful_count} / {len(ndc_data)}")
        ...
        with pharm_tab2:
            ...
            if pharmacy_records:
                ...
                for record in pharmacy_records:
                    ...
                    if med_name:
                        # FIXED: Better medication meaning lookup
                        med_meaning = (
                            med_meanings.get(med_name) or
                            med_meanings.get(med_name.upper()) or
                            med_meanings.get(med_name.lower()) or
                ...
                if medication_data:
                    ...
                    # Show how many have actual meanings vs fallback
                    meaningful_count = sum(1 for item in medication_data if not item["Medication Meaning"].startswith("Medication: "))
                    st.info(f"📋 **Medications with LLM Meanings:** {meaningful_count} / {len(medication_data)}")
...
def display_enhanced_mcid_data(mcid_data):
    """Display enhanced MCID data with professional styling"""
    if mcid_data and not mcid_data.get('error'):
        st.markdown("""
        <div class="mcid-container">
            <h3>🆔 Member Coverage ID (MCID) Data Analysis</h3>
        </div>
        """, unsafe_allow_html=True)
        
        # Display MCID match information
        mcid_match = mcid_data.get('mcid_match', {})
        if mcid_match:
            st.markdown(f"""
            <div class="mcid-match-card">
                <h4>✅ MCID Match Found</h4>
                <p><strong>MCID:</strong> {mcid_match.get('mcid', 'N/A')}</p>
                <p><strong>Match Status:</strong> <span style="color: #28a745;">Verified</span></p>
                <p><strong>Coverage Information:</strong> Available for detailed analysis</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.warning("⚠️ No MCID match data found")
        
        # Additional MCID details
        with st.expander("🔍 Detailed MCID Information", expanded=False):
            st.json(mcid_data)
    else:
        st.error("❌ MCID data not available or contains errors")
...
# ENHANCED SIDEBAR CHATBOT WITH CATEGORIZED PROMPTS AND GRAPH GENERATION - EXPANDED BY DEFAULT
with st.sidebar:
    if st.session_state.analysis_results and st.session_state.analysis_results.get("chatbot_ready", False) and st.session_state.chatbot_context:
        ...
        # CATEGORIZED SUGGESTED PROMPTS SECTION - MOVED TO TOP
        ...
        # Chat input MOVED TO AFTER PROMPTS
        st.markdown("---")
        ...
        # Chat history MOVED TO AFTER INPUT - WITH ENHANCED STYLING
        st.markdown("---")
        ...
        if st.session_state.chatbot_messages:
            ...
            for message in st.session_state.chatbot_messages:
                ...
                if message["role"] == "assistant":
                    ...
                    if matplotlib_code:
                        ...
                        if text_content.strip():
                            ...
                            <div class="chat-message {role_class}">
                                <strong>{role_icon} Assistant:</strong><br>
                                {text_content.strip()}
                            </div>
                            ''', unsafe_allow_html=True)
                    ...
                    st.markdown(f'''
                    <div class="chat-message {role_class}">
                        <strong>{role_icon} You:</strong><br>
                        {message["content"]}
                    </div>
                    ''', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
