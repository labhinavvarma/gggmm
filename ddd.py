...
class EpisodicMemoryManager:
    ...
    def _extract_mcid_list(self, deidentified_mcid: Dict[str, Any]) -> str:
        ...
        try:
            # Navigate the MCID data structure to find 'mcidlist' attribute
            ...
            # Recursive function to find 'mcidlist' attribute specifically
            def find_mcidlist_recursive(data: Any) -> str:
                if isinstance(data, dict):
                    # Look specifically for 'mcidlist' key
                    if 'mcidlist' in data and data['mcidlist']:
                        mcid_value = str(data['mcidlist']).strip()
                        if mcid_value and mcid_value != "[MASKED_NAME]":
                            logger.info(f"Found mcidlist: {mcid_value}")
                            return mcid_value
                    
                    # Continue recursive search
                    for value in data.values():
                        result = find_mcidlist_recursive(value)
                        if result:
                            return result
                
                elif isinstance(data, list):
                    for item in data:
                        result = find_mcidlist_recursive(item)
                        if result:
                            return result
                
                return ""
            
            mcid = find_mcidlist_recursive(mcid_data)
            
            if mcid:
                logger.info(f"Extracted mcidlist: {mcid}")
            else:
                logger.warning("No 'mcidlist' attribute found in deidentified data")
    ...
    def _load_existing_memory(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """Load existing episodic memory file with simplified structure validation"""
        try:
