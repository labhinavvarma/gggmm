...
if st.session_state.analysis_results and not st.session_state.analysis_running:
    ...
    with st.expander("❤️ Heart Attack Risk Prediction", expanded=False):
        ...
        if heart_attack_prediction and not heart_attack_prediction.get('error'):
            # Main risk display - REMOVED CONFIDENCE AND ENHANCED
            ...
            # Remove "Confidence:" from the display if it exists
            if "Confidence:" in combined_display:
                combined_display = combined_display.split("Confidence:")[0].strip()
            
            # Enhanced risk display with more elegant styling
            ...
            <div class="heart-risk-display">
                ...
                <div class="elegant-risk-text">
                    Advanced machine learning analysis indicates cardiovascular risk assessment based on comprehensive health data evaluation.
                </div>
            ...
            if heart_attack_features:
                ...
                if isinstance(heart_attack_features, dict):
                    factor_cols = st.columns(min(len(heart_attack_features), 4))
                    for i, (factor, value) in enumerate(heart_attack_features.items()):
                        if i < 4:  # Limit to 4 columns
                            with factor_cols[i]:
                                # Determine color based on factor name and value
                                if factor.lower() in ['age', 'diabetes', 'smoking', 'high_bp'] and value == 1:
                                    factor_color = '#dc3545'
                                else:
                                    factor_color = '#28a745'
                                
                                st.markdown(f"""
                                <div style="text-align: center; padding: 1rem; background: white; border-radius: 10px; border-left: 4px solid {factor_color}; margin: 0.5rem 0;">
                                    <h4 style="margin: 0; color: #2c3e50;">{factor.replace('_', ' ').title()}</h4>
                                    <p style="margin: 0; color: {factor_color}; font-weight: bold; font-size: 1.2rem;">{value}</p>
                                </div>
                                """, unsafe_allow_html=True)
        else:
            st.warning("⚠️ Heart attack risk prediction not available.")

# Test Graph Generation Section (Optional - can be removed in production)
if st.session_state.analysis_results and st.session_state.chatbot_context:
    with st.expander("🧪 Test Graph Generation (Development Only)", expanded=False):
        st.markdown("### Test Enhanced Graph Generation")
...
plt.bar(categories, values, color=['green', 'red', 'yellow'])
...
""",
            ...
            """
import matplotlib.pyplot as plt
labels = medication_list[:4] if len(medication_list) > 0 else ['Med A', 'Med B', 'Med C']
sizes = [30, 25, 20, 25]
plt.figure(figsize=(8, 6))
...
# Risk factors
risk_names = list(risk_factors.keys())
risk_vals = list(risk_factors.values())
ax1.bar(risk_names, risk_vals)
ax1.set_title('Risk Factors')

# Heart risk gauge
risk = heart_risk_score
ax2.pie([risk, 1-risk], labels=['Risk', 'Safe'], colors=['red', 'green'])
ax2.set_title(f'Heart Risk: {risk:.1%}')

# Medications
if len(medication_list) > 0:
    ax3.barh(medication_list[:5], range(1, len(medication_list[:5])+1))
    ax3.set_title('Medications')
else:
    ax3.text(0.5, 0.5, 'No medication data', ha='center', va='center')
    ax3.set_title('Medications')
...
"""
        ...
        for i, code in enumerate(sample_codes):
            if st.button(f"Test Sample Chart {i+1}", key=f"test_chart_{i}"):
                with st.spinner(f"Testing chart {i+1}..."):
                    img_buffer = execute_matplotlib_code_enhanced_stability(code)
                    if img_buffer:
                        st.image(img_buffer, caption=f"Test Chart {i+1}")
                        st.success(f"✅ Test chart {i+1} generated successfully!")
                    else:
                        st.error(f"❌ Test chart {i+1} failed")
