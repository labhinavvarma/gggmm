...
async def force_current_search(query: str, ctx: Context) -> str:
    ...
    try:
        ...
        if not search_results:
            return f"⚠️ LIVE SEARCH FAILED: Could not find current web results for '{query}'. Please check your internet connection or try a different search term."
        
        # Fetch content from top results
        content_results = []
        for i, result in enumerate(search_results[:3]):
            try:
                await ctx.info(f"📄 Fetching live content from: {result.title}")
                ...
                await ctx.warning(f"Failed to fetch {result.link}: {str(e)}")
                continue
        
        if not content_results:
            return f"⚠️ CONTENT FETCH FAILED: Found search results but could not read current content from websites. Search results found: {len(search_results)}"
        
        # Format response with current data emphasis
        response = f"🌐 LIVE WEB SEARCH RESULTS (Retrieved: {current_date})\n"
        response += f"Query: '{query}'\n"
        response += f"Found {len(search_results)} results, analyzed {len(content_results)} current sources:\n\n"
        
        for i, content_result in enumerate(content_results, 1):
            response += f"### {i}. {content_result['title']}\n"
            response += f"**URL:** {content_result['url']}\n"
            response += f"**Snippet:** {content_result['snippet']}\n"
            response += f"**Current Content:** {content_result['content']}\n\n"
            response += "---\n"
        
        response += f"\n✅ This information was retrieved live from the web on {current_date}"
        response += f"\n📊 Sources analyzed: {len(content_results)}"
...
async def search_prompt(query: str) -> List[Message]:
    return [
        {
            "role": "user",
            "content": f"""You are a web research expert who MUST use live web search tools to provide current, up-to-date information.

 CRITICAL RULES - NEVER VIOLATE THESE:
1. You MUST NOT use your training data for ANY current events, recent news, or time-sensitive information
2. You MUST use web search tools FIRST before answering ANY question about current topics
3. If you cannot access current web data, say "I cannot provide current information without web access"
4. NEVER guess or use outdated information from your training data
 MANDATORY WORKFLOW FOR ALL CURRENT QUERIES:
Step 1: ALWAYS start by saying "Searching the web for current information about [topic]..."
Step 2: Use force_current_search for current/recent topics OR search_and_analyze for general topics
Step 3: Only answer based on the content you actually retrieved from websites TODAY
Step 4: Include "Information retrieved on: [today's date]" and all source URLs
TOOLS YOU MUST USE (in order of preference):
1. force_current_search: Use for current events, recent news, latest developments, today's info
2. search_and_analyze: Use for general topics that need current web verification
3. duckduckgo_search + fetch_content: Manual workflow if above tools fail
 WHEN TO USE force_current_search:
- Any query with words like: "latest", "recent", "current", "today", "now", "2024", "this year"
- News, stock prices, weather, recent events, current statistics
- Technology developments, market trends, recent research
 WHAT YOU MUST NEVER DO:
- Answer current questions without using web tools first
- Use training data for anything that could have changed after your training
- Provide information without web verification for current topics
- Skip the web search step
 CORRECT RESPONSE PATTERN:
"Searching the web for current information about [topic]...

[I will now use force_current_search or search_and_analyze]

Based on my live web search conducted today, here's what I found:

[Detailed analysis of actual current web content]

Sources verified today:
- [Website Name] - [URL]
- [Website Name] - [URL]

Information retrieved on: [Current date]"

Your query: {query}

NOW: Determine if this needs current information and use the appropriate web search tool BEFORE answering!"""
        }
    ]
