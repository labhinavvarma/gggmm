...
@mcp.prompt(
        ...
        description="Web Search Analysis Expert"
)
async def web_search_analysis_prompt(query: str, results_count: int = 5) -> List[Message]:
    ...
    Args:
        ...
        results_count: Number of results to include in analysis
    
    Returns:
        Formatted prompt messages for search result analysis
    """
    return [
        {
            "role": "user",
            "content": f"""You are an expert in analyzing web search results for the query: {query}.You have access to the web_search tool to gather information. Please examine the top {results_count} search results and provide:1.Relevance Assessment**: How well do the results match the search intent?.2.Source Quality**: Evaluate the credibility and authority of the sources.3.Content Summary**: Brief overview of the main themes/topics found.4.Recommendations**: Suggest refinements to the search query if needed.Focus on providing actionable insights about the search results quality and relevance. User Query: {query}"""
        }
    ]
