...
# Simple Direct Web Search Prompt
@mcp.prompt(
...
async def web_search_analysis_prompt(query: str, results_count: int = 5) -> List[Message]:
    """
    Simple direct prompt that instructs to search for current information.
    ...
    return [
        {
            "role": "user",
            "content": f"""I need current, up-to-date information about: "{query}"

Your training data may be outdated. Please use the available web search tools to find current information about this topic.

Search query: {query}

Provide the most recent and accurate information available."""
        }
    ]
