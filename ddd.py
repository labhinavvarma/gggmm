...
"""
Streamlit Chat UI for Neo4j + LangGraph + Cortex
- Sends user queries to FastAPI /chat endpoint.
- Shows both the agent's reasoning and the final answer.
"""
...
for role, message in reversed(st.session_state.messages):
    if role == "user":
        ...
        st.markdown(f"<span style='color:grey;font-size:12px'><b>Trace:</b> {message}</span>", unsafe_allow_html=True)
