    ...
    def _prepare_summarized_context(self, pharmacy_data: Dict[str, Any],
        ...
        try:
            ...
            if pharmacy_extraction and pharmacy_extraction.get("ndc_records"):
                ...
                # Fallback: try to extract from raw pharmacy data
                try:
                    pharmacy_str = json.dumps(pharmacy_data).lower()
                    if "lbl_nm" in pharmacy_str:
                        medications.append({"name": "medications_found_in_raw_data", "ndc": "various"})
                except:
                    pass
            ...
            unique_codes = list(dict.fromkeys([code for code in diagnosis_codes if code]))[:20]
            ...
            unique_services = list(dict.fromkeys([code for code in service_codes if code]))[:10]
            context_parts.append(f"SERVICE_CODES: {unique_services}")
            
            # 5. Summary Stats
            stats = {
                "total_medications": len(pharmacy_extraction.get("ndc_records", [])) if pharmacy_extraction else 0,
                "total_medical_records": len(medical_extraction.get("hlth_srvc_records", [])) if medical_extraction else 0,
                "unique_diagnosis_codes": len(medical_extraction.get("extraction_summary", {}).get("unique_diagnosis_codes", [])) if medical_extraction else 0
            }
