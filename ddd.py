...
def optimize_query_for_visualization(query: str, node_limit: int = 1000, unlimited_mode: bool = False) -> str:
    ...
    if unlimited_mode:
        ...
        # Check if this is a specific entity query (don't limit these aggressively)
        is_specific_query = any([
            ":" in query and "MATCH" in query.upper(),  # Has labels like :Person, :EDA
            any(label in query.upper() for label in ['EDA', 'PERSON', 'COMPANY', 'USER', 'GROUP']),
            "WHERE" in query.upper(),  # Has filtering conditions
            len(query.split()) > 10  # Complex query, likely specific
        ])
        
        # Apply reasonable limits for standard display, but be less aggressive for specific queries
        if ("MATCH" in query.upper() and 
            ...
            if "RETURN" in query.upper():
                if is_specific_query:
                    # For specific queries, use higher limit or no limit
                    limit = min(node_limit, 500) if node_limit < 500 else node_limit
                    if limit > 100:  # Only add limit if it's reasonable
                        query += f" LIMIT {limit}"
                else:
                    # Use smaller limits for generic queries
...
def smart_question_preprocessor(question: str) -> str:
    ...
    # Enhanced entity-specific preprocessing - DON'T change these, they're specific!
    entity_patterns = {
        # Keep specific entity requests intact - these shouldn't be changed
        r'\beda\b.*\b(group|team|department)\b': question,  # EDA group requests
        r'\beda\b.*\b(relationship|connection|network)\b': question,  # EDA relationship requests
    ...
    for pattern, keep_original in entity_patterns.items():
        if re.search(pattern, normalized, re.IGNORECASE):
            ...
            return question  # Return original question for specific entity requests
    
    # Expand abbreviated questions only if they're not specific entity requests
...
# Enhanced system message with comprehensive examples and better specific entity handling
...
def enhanced_parse_llm_output(llm_output):
    ...
    if not tool or not query:
        ...
        if not query and tool != "get_neo4j_schema":
            if tool == "read_neo4j_cypher":
                # Enhanced entity detection for specific queries
                lower_output = llm_output.lower()
                
                # Check for specific entity mentions with relationship requests
                if any(word in lower_output for word in ['eda', 'eda group', 'eda team']):
                    if any(word in lower_output for word in ['relationship', 'connection', 'network', 'connected']):
                        query = "MATCH (n:EDA) OPTIONAL MATCH (n)-[r]-(m) RETURN n, r, m"
                    else:
                        query = "MATCH (n:EDA) RETURN n"
                    ...
                    if any(word in lower_output for word in ['relationship', 'connection', 'network', 'connected']):
                        ...
                        query = "MATCH (n:Person) RETURN n LIMIT 50"
                ...
                elif any(word in lower_output for word in ['department', 'dept']):
                    if any(word in lower_output for word in ['relationship', 'connection', 'network', 'connected']):
                        query = "MATCH (n:Department) OPTIONAL MATCH (n)-[r]-(m) RETURN n, r, m"
                    else:
                        query = "MATCH (n:Department) RETURN n"
                elif any(word in lower_output for word in ['group', 'team']):
                    if any(word in lower_output for word in ['relationship', 'connection', 'network', 'connected']):
                        query = "MATCH (n:Group) OPTIONAL MATCH (n)-[r]-(m) RETURN n, r, m"
                    else:
                ...
                # Count queries
                elif any(word in lower_output for word in ['count', 'how many', 'number', 'total', 'size']):
                    query = "MATCH (n) RETURN count(n) as TotalNodes"
    ...
    if query and tool != "get_neo4j_schema":
        ...
        # Enhanced LIMIT logic - be smarter about when to add limits
        if (tool == "read_neo4j_cypher" and 
            ...
            # Check if this is a specific entity query that shouldn't be heavily limited
            is_specific_entity_query = any([
                ":" in query,  # Has node labels like :EDA, :Person
                any(entity in query.upper() for entity in ['EDA', 'PERSON', 'COMPANY', 'USER', 'GROUP', 'DEPARTMENT']),
                "OPTIONAL MATCH" in query.upper(),  # Complex relationship query
                len(query.split()) > 8  # Complex multi-part query
            ])
            
            if is_specific_entity_query:
                # Don't add LIMIT for specific entity queries, or use a high limit
                if "OPTIONAL MATCH" not in query.upper():  # Only add limit if it's not a relationship query
                    query += " LIMIT 100"  # Higher limit for specific entities
                # If it already has OPTIONAL MATCH for relationships, don't add LIMIT at all
            else:
                # Add standard limit for generic queries
                query += " LIMIT 25"
