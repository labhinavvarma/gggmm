...
class HealthAnalysisAgent:
    ...
    def _safe_string_handler(self, response: Any) -> str:
        """Safely convert any response to string and handle common string operations"""
        try:
            if isinstance(response, dict):
                # If it's a dict, try to get 'response' key first, then convert to string
            ...
            logger.error(f"Error in safe string handling: {e}")
            return "Response processing error"

    def _safe_string_check(self, text: Any, check_value: str) -> bool:
        """Safely check if text contains or starts with a value"""
        try:
            safe_text = self._safe_string_handler(text)
            return check_value.lower() in safe_text.lower()
        except Exception as e:
            logger.error(f"Error in safe string check: {e}")
            return False
...
[Provide detailed analysis using all available deidentified medical and pharmacy claims data]"""
    ...
    def format_chat_response(self, api_response: Any, user_query: str, chat_history: List[Dict[str, str]], session_id: str = None) -> Dict[str, Any]:
        """Format chat response with consistent graph detection flags"""
        try:
            # Generate session ID if not provided
            if not session_id:
                session_id = f"health_format_{int(time.time())}_{uuid.uuid4().hex[:8]}"
            ...
            if isinstance(api_response, dict):
                ...
                response_text = str(api_response)
                # Detect graph content in string response - fallback if api_integrator method not available
                try:
                    ...
                    logger.warning("_detect_graph_content method not available, using fallback detection")
                    has_code = "```python" in response_text or "```" in response_text
                    graph_flags = {
                        "graphstart": 1 if has_code else 0,
                        "graph": has_code,
                        "graph_type": "matplotlib" if has_code else None
                    }
                success = True
            ...
            fallback_session_id = session_id or f"error_format_{int(time.time())}_{uuid.uuid4().hex[:8]}"
