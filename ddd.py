...
if show_server_info:
 ...
 # Display Prompts
    ...
    # Updated prompt types with Web Search and test prompts
    ...
    # Updated prompt mapping with new test prompts
    prompt_map = {
    ...
    # Updated examples with test prompts
    examples = {
        ...
        async def process_query(query_text):
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    if prompt_type in ["Web Search", "Test Tool", "Diagnostic"]:
                    ...
                    if prompt_type == "Web Search" and web_search_available:
                        ...
                        try:
                            ...
                            web_search_tool = next((tool for tool in available_tools if tool.name == 'web_search'), None)
                            if web_search_tool:
                                st.write("✅ **Found web_search tool object**")
                                # Try to invoke it directly - this might not work but let's see
                                try:
                                    # Check if tool has a direct invoke method
                                    if hasattr(web_search_tool, 'invoke'):
                                        test_result = await web_search_tool.invoke({"query": query_text, "limit": 3})
                                        ...
                                        st.write(f"Tool methods: {[method for method in dir(web_search_tool) if not method.startswith('_')]}")
                                except Exception as direct_error:
                                    st.write(f"⚠️ **Direct invocation failed**: {direct_error}")
                            else:
                                st.write("❌ **Could not find web_search tool object**")
                        except Exception as tool_error:
                            st.write(f"❌ **Tool test setup failed**: {tool_error}")
                    
                    # Enhanced agent monitoring for tool calls
                    if prompt_type in ["Web Search", "Test Tool", "Diagnostic"]:
                        st.write("🔍 **Agent starting with custom tool format...**")
                    ...
                    if prompt_type in ["Web Search", "Test Tool", "Diagnostic"]:
                        ...
                        elif any(keyword in str(result).lower() for keyword in ['search results', 'found', 'information']):
                            st.write("✅ **Search results detected!**")
                        else:
