...
# Display minimized deidentified output if available
if st.session_state.analysis_results and st.session_state.chatbot_ready:
    st.markdown('<div style="padding: 0 1.5rem;">', unsafe_allow_html=True)
    results = st.session_state.analysis_results
    deidentified_data = results.get("chatbot_context", {})
    
    with st.expander("🔒 Minimized Deidentified Output", expanded=False):
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**🏥 Medical Data Summary:**")
            medical_data = deidentified_data.get("deidentified_medical", {})
            if medical_data and not medical_data.get("error"):
                st.json({
                    "patient_age": medical_data.get("src_mbr_age", "unknown"),
                    "zip_code": medical_data.get("src_mbr_zip_cd", "12345"),
                    "record_count": medical_data.get("record_count", 0),
                    "status": medical_data.get("status", "processed")
                })
            else:
                st.warning("Medical data not available")
        
        with col2:
            st.markdown("**💊 Pharmacy Data Summary:**")
            pharmacy_data = deidentified_data.get("deidentified_pharmacy", {})
            if pharmacy_data and not pharmacy_data.get("error"):
                st.json({
                    "record_count": pharmacy_data.get("record_count", 0),
                    "status": pharmacy_data.get("status", "processed")
                })
            else:
...
# Chat input area with refresh button inline
    ...
    if parse_patient_command(user_input) and not st.session_state.chatbot_ready:
        ...
        with st.spinner("🔄 Processing patient analysis through all nodes..."):
            try:
                ...
                st.session_state.chat_messages.append({
                    "role": "system", 
                    "content": "🚀 Starting patient analysis: Parse Data → LLM calls MCP Tools → Deidentify → Extract → Entities → Trajectory → Summary → Chatbot"
                })
